...
In this section, we will develop an \blue{interpreter} for this language.  This is our most ambitious
example:  In contrast to the calculator of Section \ref{chapter:ply} and the differentiator of Section
...
\item Line 8 -- 12 define the variable \texttt{stmnt}.  The five alternatives correspond one to one to the
      ...
      The keywords \texttt{"if"} and \texttt{"while"} are written as \blue{anonymous literals}, i.e.~in
...
\item Line 38 -- 40 import three token definitions from the library \texttt{common}.  In contrast to the
      calculator of Section \ref{chapter:ply}, we import the token \texttt{INT} and rename it to
...
\item First, we check if \texttt{stmnt} is a list of statements.  A statement is a list of statements
      ...
      \texttt{execute\_tuple}.  Observe that the pattern in line 3 uses a \blue{star pattern}:  The
...
\\[0.2cm]
\hspace*{1.3cm}
\href{https://github.com/karlstroetmann/Formal-Languages/blob/master/Python/Chapter-07-Lark/03-Interpreter.ipynb}{Python/Chapter-07-Lark/03-Interpreter.ipynb}
\\[0.2cm]
implements the interpreter that has been discussed in this section.
...
\\[0.2cm]
\hspace*{1.3cm}
\href{https://github.com/karlstroetmann/Formal-Languages/blob/master/Python/Chapter-07-Lark/Interpreter-Frame.ipynb}{Python/Chapter-07-Lark/Interpreter-Frame.ipynb}
\\[0.2cm]
...
\item Why is the string that contains the grammar of Section \ref{chapter:interpreter} written as a
      \blue{raw string}?
