...
syntaktische Variable auch als \emph{syntaktische Kategorie} bezeichnet.
...
\begin{eqnarray*}
  \textsl{s} & \rightarrow & \quoted{(} \textsl{s} \quoted{)}             \\
             & \mid        & \varepsilon
\end{eqnarray*}
...
\exerciseStar
Es sei $\Sigma = \{ \texttt{A}, \texttt{B} \}$.  Wir definieren die Menge $L$ als die
...
\item $L(a) = \{ w \in \Sigma^* \mid\; \exists k \in \mathbb{N}: |w| = 2 \cdot k - 1 \wedge w[k] = a \}$,
...
\item $L(b) = \{ w \in \Sigma^* \mid\; \exists k \in \mathbb{N}: |w| = 2 \cdot k - 1 \wedge w[k] = b \}$,
...
\\[0.2cm]
Der Grund ist, dass ein String $w \in L$ entweder mit einem \texttt{A} oder mit einem \texttt{B}
beginnt.  Im ersten Fall muss es zu diesem \texttt{A} ein korrespondierendes \texttt{B} geben, denn
die Anzahl der Auftreten von \texttt{A} und \texttt{B} sind gleich.  Fassen wir den Buchstaben
\texttt{A} wie eine öffnende Klammer auf und interpretieren den Buchstaben \texttt{B} als die zu
\texttt{A} korrespondierende schließende Klammer, so ist klar, dass der String, der zwischen diesen
beiden Auftreten von \texttt{A} und \texttt{B} liegt, ebenfalls gleich viele Auftreten von
\texttt{A} wie von \texttt{B} hat.  Genauso muss dies dann für den Rest des Strings gelten, der nach
dem \texttt{B} folgt.  Diese Überlegung erklärt die Regel
\\[0.2cm]
\hspace*{1.3cm}
$\textsl{s} \;\rightarrow\; \mathtt{A} s \mathtt{B} s$
\\[0.2cm]
Die Regel
\\[0.2cm]
\hspace*{1.3cm}
$\textsl{s} \;\rightarrow\; \mathtt{B} s \mathtt{A} s$
\\[0.2cm]
lässt sich in analoger Weise erklären,  wenn wir den Buchstaben \texttt{B} als öffnende Klammer und
\texttt{A} als schließende Klammer interpretieren. 
...
$L(x) = \bigl\{ \omega \in \Sigma^* \mid \omega \in L \;\wedge\; \forall \sigma \preceq \omega : 
                  \textsl{count}(\sigma,\mathtt{B}) \leq \textsl{count}(\sigma, \mathtt{A}) \bigr\}$
\quad und \\[0.2cm]
\hspace*{1.3cm}
$L(y) = \bigl\{ \omega \in \Sigma^* \mid \omega \in L \;\wedge\; \forall \sigma \preceq \omega : 
                  \textsl{count}(\sigma,\mathtt{A}) \leq \textsl{count}(\sigma, \mathtt{B}) \bigr\}$.
\\[0.2cm]
...
$
\\[0.2cm]
definiert, so hat eine Ableitung von $a$, bei der zunächst immer die Variable $a$ ersetzt
wird, die Form 
\\[0.2cm]
\hspace*{1.3cm}
