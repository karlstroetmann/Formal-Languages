...
um die Syntax arithmetischer Ausdrücke beschreiben zu können.  Damit sind reguläre Sprachen dann
erst recht nicht geeignet, die Syntax von Programmier-Sprachen zu beschreiben.  Hierfür ist ein
...
die \href{http://en.wikipedia.org/wiki/Context-free_language}{\emph{kontextfreien}} Sprachen.  Diese basieren auf
...
es sich bei den kontextfreien Sprachen genau wie bei den regulären Sprachen auch um
...
\end{itemize}
Der Parser bekommt dann vom Scanner eine Folge solcher Token und hat die Aufgabe, daraus
einen sogenannten \emph{Syntax-Baum} zu bauen.  Dazu bedient sich der Parser einer
...
\begin{eqnarray*}
  ...
  \textsl{Factor}    & \rightarrow & \textsl{Variable}                               \\
  \textsl{Factor}    & \rightarrow & \textsl{Number} 
\end{eqnarray*}
Die Ausdrücke auf der linken Seite einer Grammatik-Regel bezeichnen wir als
\emph{syntaktische Variablen} oder auch als \emph{Nicht-Terminale}.  In der Literatur
finden Sie hierfür auch den Begriff \emph{syntaktische Kategorie}.
In dem Beispiel sind \textsl{ArithExpr}, \textsl{Product} und \textsl{Factor} die 
syntaktischen Variablen.  Die restlichen Ausdrücke, in unserem Fall also \textsl{Number} und
die Zeichen 
...
\item Token wie \textsl{Number} oder \textsl{Variable} ist zusätzlich ein Wert zugeordnet.
      Im Falle von \textsl{Number} ist dies eine Zahl, im Falle von \textsl{Variable}
...
\item $T$ ist eine Menge von Namen, die wir als \emph{Terminale} bezeichnen.  
      ...
      \\[0.2cm]
      \hspace*{1.3cm}
      $T = \{ \textsl{Number}, \quoted{+}, \quoted{-}, \quoted{*}, \quoted{/}, \quoted{(}, \quoted{)} \}$
...
& \Rightarrow & \textsl{Number} \quoted{*} \textsl{Factor} \quoted{+} \textsl{Product}  \\
& \Rightarrow & \textsl{Number} \quoted{*} \textsl{Number} \quoted{+} \textsl{Product}  \\
& \Rightarrow & \textsl{Number} \quoted{*} \textsl{Number} \quoted{+} \textsl{Factor}   \\
& \Rightarrow & \textsl{Number} \quoted{*} \textsl{Number} \quoted{+} \textsl{Number}   
\end{eqnarray*}
Damit haben wir also gezeigt, dass
\\[0.2cm]
\hspace*{1.3cm}
$\textsl{ArithExpr} \Rightarrow^* \textsl{Number} \quoted{*} \textsl{Number} \quoted{+} \textsl{Number}$
\\[0.2cm]
oder genauer
\\[0.2cm]
\hspace*{1.3cm}
$\textsl{ArithExpr} \Rightarrow^8 \textsl{Number} \quoted{*} \textsl{Number} \quoted{+} \textsl{Number}$
\\[0.2cm]
gilt.  Ersetzen wir hier das Terminal \textsl{Number} durch verschiedene Zahlen, so haben wir damit
...
\\[0.2cm]
so werden wir gelich sehen, dass
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
\hspace*{1.3cm}
$G = \bigl\langle \{\textsl{Expr}\}, \{ \textsl{Number}, \quoted{+}, \quoted{-}, \quoted{*}, \quoted{/}, \quoted{(}, \quoted{)} \}, R, \textsl{Expr} \bigr\rangle$,
\\[0.2cm]
