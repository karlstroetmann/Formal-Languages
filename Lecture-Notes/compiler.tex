...
\item Der Code-Generator übersetzt schließlich den Parse-Baum in eine Folge von
      Assembler-Befehlen, die der Syntax von \textsl{Jasmin} entsprechen.  
...
\begin{enumerate}
\item \textsc{Id} ist eine Abkürzung für \blue{Identifier} und steht für eine Folge von Ziffern, Buchstaben und
      dem Unterstrich, die mit einem Buchstaben beginnt.  Eine \textsc{Id} bezeichnet entweder eine Variable oder
...
\item Daneben haben wir eine Reihe von Operatoren wie z.B.~\quoted{+},\quoted{-}, etc., sowie Schlüsselwörter
      wie beispielsweise \squoted{if}, \squoted{else} und \squoted{while}.
\end{enumerate}
...
\item Die Grammatik hat das \emph{Dangling-Else-Problem}.
      ...
      Wir werden das Problem bei dem Parser, den wir nun vorstellen, durch die Verwendung von
      Operator-Präzedenzen lösen. 
...
\end{Verbatim}
\vspace*{-0.3cm}
\caption{Ein einfaches \textsc{Integer}-\mytt{C}-Programm zur Berechnung der Summe $\sum\limits_{i=1}^{6 \cdot 6} i$.}
\label{fig:sum.c}
\end{figure}
