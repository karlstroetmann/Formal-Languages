...
The figures \ref{fig:Compiler.ipynb:lex1} and \ref{fig:Compiler.ipynb:lex2} shows the scanner.  The scanner
recognizes the operators, keywords, identifiers, and numbers.  Also the scanner is largely similar to those
scanners that we have already seen before, there are a few points worth mentioning.
...
\end{Verbatim}
\vspace*{-0.3cm}
\caption{The scanner for \textsl{Integer}-\texttt{C}, part 1.}
\label{fig:Compiler.ipynb:lex1}
\end{figure}
...
\item While we do not need to declare tokens for those operators that consist of just one token, 
      ...
      \quoted{==} is represented by the token \texttt{'EQ'} and defined in line 11--13.
\item Our programming language supports single line comments that start with the string \quoted{//} and extend
      to the end of the line.  These comments are implemented via the token \texttt{COMMENT} that is defined in
      line 35--37.  Note that the function \texttt{t\_COMMENT} does not return a value.  Therefore,
      these comments are simply discarded.
\item The most interesting aspect is the implementation the keywords like \quoted{while} or \quoted{return}.
      Note that we have defined separate tokens for all of the keywords but we have not implemented any
      functions for these keywords.  For example, we have not defined a function with the name
      \texttt{t\_return}.
      The reason is that, syntactically, all of the keywords are identifiers and are recognized by the regular
      expression
      \\[0.2cm]
      \hspace*{1.3cm}
      \texttt{r'[a-zA-Z][a-zA-Z0-9\_]*'}
      \\[0.2cm]
      that is used for recognizing identifiers.  However, the function \texttt{t\_ID} that recognizes
      identifiers does not immediately return the token \texttt{'ID'} when it has scanned the name of an
      identifier.  Rather, it first checks whether this name is a predefined keyword.  The predefined keywords
      are the key of the dictionary \texttt{Keywords} that is defined in line 38-43.  The values of the
      keywords in this dictionary are the token types.  Therefore, the function \texttt{t\_ID} sets the
      \blue{token type} of the token that is returned to the value stored in the dictionary \texttt{Keywords}.
      In case a name is not defined in this dictionary, the token type is set to \texttt{'ID'}.
\end{enumerate}
...
\end{Verbatim}
\vspace*{-0.3cm}
\caption{The  Scanner for \textsl{Integer}-\texttt{C}, part 2.}
\label{fig:Compiler.ipynb:lex2}
\end{figure}
