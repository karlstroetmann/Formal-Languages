...
\begin{enumerate}
\item The grammar has the \blue{Dangling-Else Problem}\index{dangling else problem}.  We will solve this problem via operator precedences. 
...
\item The most interesting aspect is the implementation of keywords like \quoted{while} or \quoted{return}.
      ...
      are the key of the dictionary \mytt{Keywords} that is defined in lines 18 and 19.  The values of the
...
\item The definition of a list of parameters is quite involved, because parameter lists may be empty and,
      ...
      \begin{tcolorbox}[colback=sepia, colframe=blue, title=\texttt{param\_list}]
      \begin{verbatim}
        param_list 
             : /* epsilon */
             | 'int' ID
...
\item The grammar rules for a list of declarations are as follows:
      ...
      \begin{tcolorbox}[colback=sepia, colframe=blue, title=\texttt{decl\_list}]
      \begin{verbatim}
      decl_list
          : /* epsilon */
          | 'int' ID ';' decl_list
...
\item The grammar rules for a list of statements are as follows:
      ...
      \begin{tcolorbox}[colback=sepia, colframe=blue, title=\texttt{stmnt\_list}]
      \begin{verbatim}
      stmnt_list
          : /* epsilon */
          | stmnt stmnt_list
...
\begin{tcolorbox}[colback=sepia, colframe=blue, title=\texttt{stmnt}]
\begin{verbatim}
        stmnt : '{' stmnt_list '}'
              | ID '=' expr ';'
              | 'if' '(' bool_expr ')' stmnt
...
\begin{tcolorbox}[colback=sepia, colframe=blue, title=\texttt{bool\_expr}]
\begin{verbatim}
...
\begin{tcolorbox}[colback=sepia, colframe=blue, title=\texttt{expr}]
\begin{verbatim}
        expr : expr '+' expr
             | expr '-' expr
             | expr '*' expr
...
\begin{tcolorbox}[colback=sepia, colframe=blue, title=\texttt{expr\_list}]
\begin{verbatim}
        expr_list : /* epsilon */
                  | expr
                  | expr ',' ne_expr_list
