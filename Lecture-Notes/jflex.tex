...
\emph{Token} aufspaltet.  Ein Token ist dabei eine Gruppe logisch zusammenhängender Buchstaben.
Beispielsweise spaltet der Scanner, der für einen \texttt{C}-Compiler eingesetzt wird, den
Programmtext in die folgenden Token auf: 
\begin{enumerate}
...
\item Konstanten, wobei es in der Sprache \texttt{C} drei Arten von Konstanten gibt:
      ...
      \end{enumerate}
\item Namen, die als Bezeichner für Variablen, Funktionen, oder Typen fungieren.
\item Kommentare.
...
eingesetzt werden, auf denen \textsl{Java} zur Verfügung steht.  Wir werden in dieser Vorlesung die
Version \texttt{1.6.0} von \textsl{JFlex} verwenden.

Im nächsten Abschnitt besprechen wir die Struktur einer \textsl{JFlex}-Eingabe-Datei und
...
muss, voneinander getrennt werden.
\begin{enumerate}
\item Der erste Teil ist der \emph{Benutzer-Code}.  Er besteht aus
      ...
      Allerdings ist es sinnvoller, solche Klassen in separaten Dateien zu definieren.
...
\item Der zweite Teil ist der \emph{Options-Teil}.  Dieser Teil enthält die Spezifikation 
      verschiedener Optionen, sowie eventuell die Deklaration von Member-Variablen und Methoden 
...
\item Die Zeilen 18 bis 20 enthalten die Regeln unseres Scanners.  Eine Regel hat die Form
      ...
      wird.
\item In Zeile 18 steht der reguläre Ausdruck ``\texttt{0[0-9]+}'' für solche Strings, die mit
      ...
      zum Kleene-Operator ``\texttt{*}'', verlangt aber zusätzlich, dass mindestens eine weitere
      Ziffer aus dem Bereich ``\texttt{[0-9]}'' auftritt.  Wir werden die Syntax, mit der in
      \textsl{JFlex} reguläre Ausdrücke beschrieben werden, später noch im Detail diskutieren.
...
\item In Zeile 19 spezifiziert der reguläre Ausdruck ``\texttt{0|[1-9][0-9]*}'' eine natürliche Zahl.
...
\item In Zeile 20 spezifiziert der reguläre Ausdruck ``\texttt{.|\symbol{92}R}'' ein beliebiges
      Zeichen.  Der reguläre Ausdruck ``\texttt{.}'' spezifiziert dabei ein Zeichen, das
      von einem Unicode-Zeilenumbruch verschieden ist, während ``\texttt{\symbol{92}R}'' für einen
      ...
      In dem Ausdruck  ``\texttt{.|\symbol{92}R}'' steht der Operator ``\texttt{|}''  für die
      Alternative.  Im 
...
\\[0.2cm]
aufrufen, wobei \texttt{input.txt} den Namen der zu scannenden Datei angibt.  Für die in Abbildung
\ref{fig:input.txt} gezeigte Datei erhalten wir dann die Ausgabe
\\[0.2cm]
\hspace*{1.3cm}
\texttt{Total: 46}.
\\[0.2cm]
Damit wir die obigen Befehle nicht jedes mal neu eingeben müssen, wenn wir die verwendete
\textsl{JFlex}-Spezifikation ändern, empfiehlt es sich, ein 
...
aus der in der Datei \texttt{count.jflex} enthaltenen Spezifikation erstellen können.
...
\begin{enumerate}
\item Die erste Zeile der Datei \texttt{Makefile} enthält zunächst das \emph{Ziel}, das erstellt werden
      soll.  In diesem Fall haben wir das Ziel mit dem Namen \texttt{run} bezeichnet.  Hinter dem
      Ziel stehen die Abhängigkeiten.  In unserem Fall sind dies zwei Dateinamen: Zum einen die Datei
      \texttt{Count.class} in dem Verzeichnis \texttt{count}, zum anderen die Datei
      \texttt{input.txt}.  Diese beiden Dateien müssen vorhanden sein, damit das Ziel erreicht
      werden kann.
 
      Die zweite Zeile gibt den Befehl an, mit dem das Ziel erreicht werden kann.  In diesem Fall
      ist das die Ausführung eines Java-Programms, dass den Namen der Datei \texttt{input.txt} als
      Eingabe erhält.

      \textbf{Wichtig} ist hier, dass diese zweite Zeile durch einen Tabulator eingerückt werden muss.
      Würden statt dessen Leerzeichen verwendet, dann würde das Makefile nicht funktionieren!
\item Die Zeile 4 gibt an, wie die Datei \texttt{Count.class} erzeugt werden kann.
      ...
      benötigt wird.  Die Erzeugung von \texttt{Count.class} geschieht dann durch den Aufruf des
      \textsl{Java}-Compilers in Zeile 5.
\item Schließlich beschreibt die Regel in Zeile 7 und Zeile 8, dass zur Erzeugung der Datei
      \texttt{Count.java} die Datei \texttt{count.jflex} benötigt wird und gibt weiter den Befehl
...
``\texttt{make}'' in der Kommando-Zeile auszuführen.  Falls der Scanner, der in Datei
\texttt{Count.class} enthalten ist, bereits vorhanden wäre, würde \texttt{make} nur den Scanner
...
Dies ist nützlich, wenn wir später die Äquivalenz von den durch reguläre Ausdrücke spezifizierten
...
Damit können wir nun die Menge $\textsl{Regexp}\/$ der von \textsl{JFlex}\/ unterstützten regulären
...
\item $\texttt{\symbol{92}}x \in \textsl{Regexp}$ \quad 
      ...
      \item \texttt{\symbol{92}R} steht für einen beliebigen Unicode-Zeilenumbruch.
      \end{enumerate}
\item $\texttt{\symbol{92}}abc \in \textsl{Regexp}$ \quad 
      ...
      Die größte Zahl, die noch als \textsc{Ascii}-Code darstellbar ist, ist die Zahl 255.  Im
...
\item $r_1\texttt{|}r_2 \in \textsl{Regexp}$ \quad falls $r_1,r_2 \in \textsl{Regexp}$
      ...
      Der reguläre Ausdruck $r_1\texttt{|}r_2$ beschreibt sowohl die Strings, die durch $r_1$
      spezifiziert werden, als auch die Strings, die durch $r_2$ spezifiziert werden.
      Im letzten Kapitel hatten wir dafür den Operator
      ``$+$'' verwendet.
...
\item $r_1\texttt{/}r_2$ \quad falls $r_1, r_2 \in \textsl{Regexp}$
      ...
      ein Text folgen muss, welcher der Spezifikation $r_2$ genügt.  Im Unterschied zur einfachen
...
\item Innerhalb von doppelten Anführungszeichen verlieren alle Operator-Symbole bis auf das
      Operator-Symbol ``\texttt{\symbol{92}}'' ihre Bedeutung.  So beschreibt der reguläre Ausdruck 
      \\[0.2cm]
      \hspace*{1.3cm}
...
\item \textsl{JFlex} bietet zusätzlich den Negations-Operator ``\texttt{!}'' als
      ...
      regulärer Ausdrücke $r_1$ und $r_2$ zu definieren, denn aufgrund des deMorgan'schen Gesetzes
...
\item In \textsl{JFlex} können innerhalb von Bereichen bestimmte in \textsl{Java} 
      ...
      \item \texttt{[:lowercase:]} wird zurückgeführt auf die Methode
            $\texttt{isLowercase}()$.
      \end{enumerate}
\item Zusätzlich unterstützt \textsl{JFlex} die Verwendung von Unicode-Zeichen-Klassen mit Hilfe der Syntax
      \\[0.2cm]
      \hspace*{1.3cm}
      \texttt{\symbol{92}p\{\textsl{className}\}}.
      \\[0.2cm]
      Hier steht \textsl{className} für eine beliebige Unicode-Zeichen-Klasse.  Beispielsweise
      steht der Name \texttt{greek} für einen beliebigen griechischen Buchstaben.  Um in einem
      Unicode-Text nach griechischen Buchstaben zu suchen, können wir also den regulären Ausdruck
      \\[0.2cm]
      \hspace*{1.3cm}
      \texttt{\symbol{92}p\{greek\}}
      \\[0.2cm]
      verwenden.  Wollen wir eine Unicode-Zeichen-Klasse negieren, so können wir die Schreibweise
      \\[0.2cm]
      \hspace*{1.3cm}
      \texttt{\symbol{92}P\{\textsl{className}\}}
      \\[0.2cm]
      ...
      \texttt{\symbol{92}P\{greek\}}.
      \\[0.2cm]
      Die Liste aller Unicode-Zeichen-Klassen erhalten Sie, wenn \texttt{jflex} in der Form
      \\[0.2cm]
      \hspace*{1.3cm}
...
\item \texttt{\%column}
      ...
      \texttt{yycolumn} zur Verfügung gestellt.  Damit gibt \texttt{yycolumn} also die Spalte des
...
\item \texttt{\%ignorecase}
      ...
      Kleinschreibung keine Rolle.  Beispielsweise spezifiziert der reguläre Ausdruck
      ``\texttt{[a-z]+}'' dann auch auf den String ``\texttt{ABC}''.
\end{enumerate}
...
\item Alternativ können wir auch mit sogenannten \emph{Start-Zuständen} arbeiten.
      Wir werden diese letzte Möglichkeit jetzt an Hand eines weiteren Beispiels vorführen,
