...
\textsc{Antlr} unterstützt \textsc{Ebnf}-Grammatiken.
Beispielsweise werden die von regulären Ausdrücken bekannten Postfix-Operatoren ``\texttt{*}'', ``\texttt{+}''
und ``\texttt{?}'' unterstützt.  Wir demonstrieren die Verwendung dieser Operatoren, indem wir
mit Hilfe des Parser-Generators \textsc{Antlr} einen Interpreter für eine einfache Programmiersprache
entwickeln.  Abbildung \ref{fig:Pure.g4} zeigt die \textsc{Antlr}-Grammatik der Programmier-Sprache, für die wir 
...
anderes als der abstrakte Syntax-Baum, der dem ursprünglichen Programm entspricht.
...
\item Falls es sich bei dem auszuwertenden Ausdruck um einen Aufruf der Funktion \texttt{read}   handelt,
      fordern wir den Benutzer auf, eine natürliche Zahl einzugeben.  Den vom Benutzer eingegebenen String
...
\item Erweitern Sie den Interpreter um die logischen Operatoren
      ...
      und ``\texttt{!}'' für die Negation.  Dabei soll der Operator ``\texttt{!}'' am
