...
Es sei $\Sigma$ eine Alphabet, $L \subseteq \Sigma^*$ eine formale Sprache und $n\in\mathbb{N}$
eine natürlich Zahl.  Wir definieren die \emph{$n$-te Potenz} von $L$, wir schreiben $L^n$, durch
Induktion nach $n$.
\begin{enumerate}
\item[I.A.:] $n = 0$: 
...
\begin{Definition}[Kleene Closure]
  ...
  $L^n$ for all $n \in \mathbb{N}$: 
  \\[0.2cm]
  \hspace*{1.3cm}
  $L^* := \bigcup\limits_{n \in \mathbb{N}} L^n$.
  \\[0.2cm]
  Note that in these lecture notes we assume that $0 \in \mathbb{N}$. Therefore, $\varepsilon \in L^*$.
  \qed  
\end{Definition}
...
\\[0.2cm]
Formal definieren wir für einen beliebigen String $s$ und eine natürliche Zahl $n \in \mathbb{N}$
...
\begin{Definition}[reguläre Ausdrücke]
  ...
  \item $\varepsilon \in \texttt{RegExp}_\Sigma$
        ...
        \\[0.2cm]
        Beachten Sie, das die beiden Auftreten von $\varepsilon$ in der obigen Gleichung
  ...
  \item $r \in \texttt{RegExp}_\Sigma \rightarrow r^* \in \texttt{RegExp}_\Sigma$
        ...
        Aus dem regulären Ausdrücken $r$ kann mit dem Postfix-Operator
        ``$^*$'' ein neuer regulärer Ausdruck gebildet werden.  Dieser so gebildete Ausdruck
...
\item Wieder sei $\Sigma = \{ 0, 1 \}$.   Geben Sie einen regulären Ausdruck für die Sprache
      $L \subseteq \Sigma^*$ der Wörter an, in denen der Teilstring $110$ nicht auftritt.
       ...
       $w \in L(r)$.  Now if the character $0$ does not occur in the
       string $w$, then $w$ is just a bunch of zeros and therefore $w$ can be generated by the
       regular expression $(0 + 1  \cdot 0)^* \cdot 1^*$.  If the string $w$ does contain the character $1$,
       there are two cases.
       \begin{enumerate}
...
\item $(r^*)^* \doteq r^*$      
      ...
      \\[0.2cm]
      gilt $w \in L\bigl((r^*)^*\bigr)$ genau dann, wenn es ein $n \in \mathbb{N}$
      ...
      $m(i) \in \mathbb{N}$, so dass es für $j=1,\cdots, m(i)$ Wörter 
...
\\[0.2cm]
\hspace*{1.3cm}
$x = y z_1 \cdots z_m \in L(r)$ \quad für alle $m \in \mathbb{N}$ gilt.
\begin{enumerate}
\item[I.A.:] $m= 0$.
