...
Let \(G = \langle V, T, R, s \rangle\) be a context-free grammar and \(\textsl{a} \in V\).
In computing \(\textsl{Follow}(\textsl{a})\), the grammar is first \blue{augmented}
by adding the symbol \(\$\) as a new symbol to the set \(T\) of terminals and adding
the new symbol \(\widehat{s}\) to the variables.  This symbol is also the new start symbol of the grammar. To the set \(R\) of rules,
...
\textit{augmented} grammar\index{augmented grammar}. 
