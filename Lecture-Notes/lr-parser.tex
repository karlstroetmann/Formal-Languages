...
\begin{Definition}[Marked Rule]
  A \textcolor{blue}{marked rule} \index{marked rule}\glsadd{marked} of a context-free grammar $G = \langle V, T, R, s\rangle$
  is a triple 
  \\[0.2cm]
  \hspace*{1.3cm}
...
\begin{Definition}[\(\textsl{closure}(\mathcal{M})\)]\glsadd{closure}
...
grammar \(G = \langle V, T, R, s \rangle\) by introducing a new start symbol \(\widehat{s}\)\glsadd{dollar}
...
\\[0.2cm]
Next, we construct the function \(\textsl{goto}()\).\glsadd{gotoM} The definition is as follows:
\\[0.2cm]
\hspace*{1.3cm}
...
has already been recognized. This state must contain the marked rule 
\\[0.2cm]
\hspace*{1.3cm}
\(a \rightarrow \beta \bullet c\, \delta\)
\\[0.2cm]
If a \(c\) is recognized in this state, the parser can transition from this state to a state containing the
rule 
\\[0.2cm]
\hspace*{1.3cm}
...
\).
\\[0.2cm]
Before we can compute the function \textsl{action}, we need to define two auxiliary functions, 
...
\begin{Definition}[\(\textsl{First}()\)]\index{First@\textsl{First}}\glsadd{first}
...
\begin{Definition}[\(\textsl{Follow}()\)]\index{Follow@\textsl{Follow}}\glsadd{follow}
...
\item Check whether there are any conflicts.
\item Specify the function \(\textsl{action}\) for the first four states.
\end{enumerate}
...
\begin{figure}[htbp]
  ...
  \textsl{conjunction} & \rightarrow & \;\textsl{conjunction} \;\;\mathquoted{\wedge}\; \textsl{simple}   \\
...
\\[0.2cm]
\hspace*{1.3cm}
\href{https://github.com/karlstroetmann/Formal-Languages/blob/master/Python/Chapter-10/SLR-Table-Generator.ipynb}{Python/Chapter-10/SLR-Table-Generator.ipynb}
\\[0.2cm]
...
\begin{Definition}[Extended Marked Rule] \hspace*{\fill}\linebreak
  An \textcolor{blue}{extended marked rule} \index{extended marked rule}\glsadd{extmarked} (abbreviated: \textcolor{blue}{e.m.R.}) \index{e.m.R.|see{extended marked rule}}
  of a grammar 
...
\begin{Verbatim}[ frame         = lines, 
    ...
    function closure($\mathcal{M}$) \{
        ...
        while ($\mathcal{K}^{-}\!$ $\not=$ $\mathcal{K}$) \{
            $\mathcal{K}^{-}\!$ := $\mathcal{K}$;
            $\mathcal{K}$  := $\mathcal{K} \cup \bigl\{\left(c\rightarrow\bullet\gamma:\bigcup\{\textsl{First}(\delta\,t)\mid{}t\in{}L\}\right) \mid$
                         $(a\rightarrow\beta\bullet{}c\,\delta:L)\in\mathcal{K}\wedge(c\rightarrow\gamma)\in{}R\bigr\}$;
        \}
        return $\mathcal{K}$;
    \}
\end{Verbatim}
...
\\[0.2cm]
\hspace*{1.3cm}
\href{https://github.com/karlstroetmann/Formal-Languages/blob/master/Python/Chapter-10/LR-Table-Generator.ipynb}{Python/Chapter-10/LR-Table-Generator.ipynb}
\\[0.2cm]
...
\href{https://github.com/karlstroetmann/Formal-Languages/blob/master/Python/Chapter-10/Examples/c-grammar.g}{\texttt{C} grammar} 
...
\href{https://github.com/karlstroetmann/Formal-Languages/blob/master/Python/Chapter-10/Examples/c-grammar-slr-table.txt}{conflicts},
...
\href{https://github.com/karlstroetmann/Formal-Languages/blob/master/Python/Chapter-10/Examples/c-grammar-lr-table.txt}{here}.
...
\begin{figure}[htbp]
  ...
  \caption{A grammar from the Dragon Book.}
  \label{fig:dragon-book.grammar}
\end{figure}

\begin{figure}[!ht]
\centering
      \includegraphics[scale=0.5]{Abbildungen/cc-LR.eps}
      \caption{LR-goto-graph for the grammar of Figure \ref{fig:dragon-book.grammar}.}
  \label{fig:cc-LR.eps}
\end{figure}

Figure \ref{fig:cc-LR.eps} shows the so-called \blue{LR-goto-graph}\index{LR-goto-graph} for this grammar.
...
replace everywhere in $s_9$ the terminal $\text{\saquoted{\symbol{36}}}$ with the set 
 $\{ \text{\saquoted{x}}, \text{\saquoted{y}}\}$. If we define the \blue{core}\index{core of a set of extended marked rules}
...
\begin{figure}[!ht]
\centering
  \hspace*{-0.6cm} \includegraphics[scale=0.5]{Abbildungen/cc-LALR}
...
have the same core, we define the \blue{extended union}\index{extended union}\glsadd{uplus}  
...
an \blue{SLR language}\index{SLR language} if $\mathcal{L}$ can be recognized by an SLR parser.
The terms \blue{canonical LR language}\index{canonical LR language} and \blue{LALR language}\index{LALR language} are defined analogously.
...
We have defined LALR parsers by combining different states of a canonical LR parser. Therefore, it is clear
that canonical LR parsers are at least as powerful as LALR parsers. To demonstrate that canonical LR parsers
are indeed more powerful than LALR parsers, we need a grammar for which a canonical LR parser can be generated,
but not an LALR parser. Figure \ref{fig:lr-but-notlalr.g} shows such a grammar, which I have taken from the
Dragon Book. 
...
On the other hand, no one will want to program an LALR parser or a canonical LR parser by hand. Instead, you
will later use a parser generator like \simtextsc{Ply} for \textsl{Python}, \textsl{Bison} for \texttt{C}, or
\textsl{JavaCup} for \textsl{Java}.  While \simtextsc{Ply} and \textsl{JavaCup} can only generate an LALR parser,
\href{http://www.gnu.org/software/bison/manual/bison.html}{\textsl{Bison}} can also generate an LR parser. 
...
\item Are you able to compute the closure of a set of extended marked rules?
\item Define the notion of shift/reduce and reduce/reduce conflicts for both \simtextsc{Slr}-parsers and for
      \simtextsc{Lr}-parsers. 
\item Why are \simtextsc{Lr}-parser-generators more powerful than \simtextsc{Slr}-parser-generators?
\item How is the set of states reduced when we transform an \simtextsc{Lr}-parser into an \simtextsc{Lalr}-parser?
\end{enumerate}
