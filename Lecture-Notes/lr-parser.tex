...
Demgegenüber ist kann gezeigt werden, dass jede SLR-Grammatik eindeutig sein muss.  Folglich
...
\remark
As part of the resources provided with this lecture there is a
\href{https://github.com/karlstroetmann/Formal-Languages/tree/master/ANTLR4/SLR-Parser-Generator}{program}
available that, given a grammar, computes the SLR-states together with the action table for a given grammar.  
The \texttt{Makefile} of this program requires the prior installation of \href{http://www.antlr.org}{\textsc{Antlr}}.
...
\begin{Verbatim}[ frame         = lines, 
    ...
    procedure closure($\mathcal{M}$) \{
        ...
        while ($\mathcal{K}^{-}\!$ $\not=$ $\mathcal{K}$) \{
            $\mathcal{K}^{-}\!$ := $\mathcal{K}$;
            $\mathcal{K}$  := $\mathcal{K} \cup \bigl\{\left(B\rightarrow\bullet\gamma:\bigcup\{\textsl{First}(\beta{}t)\mid{}t\in{}L\}\right) \mid$ 
                         $(A\rightarrow\alpha\bullet{}B\beta:L)\in\mathcal{K} \wedge (B\rightarrow\gamma)\in{}R \bigr\}$;
        \}
        return $\mathcal{K}$;
    \}
\end{Verbatim}
...
\remark
As part of the resources provided with this lecture there is a
\href{https://github.com/karlstroetmann/Formal-Languages/tree/master/ANTLR4/LR-Parser-Generator}{program}
available that, given a grammar, computes the LR-states together with the action table for a given grammar.  
As for SLR-states, the \texttt{Makefile} of this program requires the prior installation of
\href{http://www.antlr.org}{\textsc{Antlr}}. 

\noindent
\textbf{Bemerkung}: Die Theorie der kanonischen LR-Parser geht auf Donald E.~Knuth zurück,
der diese Theorie 1965 entwickelt hat \cite{knuth:65}.

\section{LALR-Parser}
Die Zahl der Zustände eines LR-Parsers ist oft erheblich größer als die Zahl der Zustände, die ein
SLR-Parser der selben Grammatik hätte.  Anfangs, als der zur Verfügung stehenden
Haupt-Speicher der meisten Rechner noch bescheidener dimensioniert waren, als dies heute
der Fall ist, hatten LR-Parser daher eine inakzeptable 
...
LALR-Parsern in der gleichen Größenordnung wie der Speicherbedarf von SLR-Parsern.
In den heute in der Regel zur Verfügung stehenden Hauptspeichern lassen sich allerdings
auch kanonische LR-Parser mühelos unterbringen, so dass es eigentlich keinen zwingenden
