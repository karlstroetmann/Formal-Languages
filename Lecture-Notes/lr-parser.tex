...
\begin{Definition}[Marked Rule]
  A \textcolor{blue}{marked rule} \index{marked rule} of a context-free grammar $G = \langle V, T, R, s\rangle$
  is a triple 
  \\[0.2cm]
  \hspace*{1.3cm}
...
\begin{Definition}[\(\textsl{First}()\)]\index{First@\textsl{First}}
...
\begin{Definition}[\(\textsl{Follow}()\)]\index{Follow@\textsl{Follow}}
...
\begin{Definition}[Extended Marked Rule] \hspace*{\fill}\linebreak
  An \textcolor{blue}{extended marked rule} \index{extended marked rule} (abbreviated: \textcolor{blue}{e.m.R.}) \index{e.m.R.|see{extended marked rule}}
  of a grammar 
...
have the same core, we define the \blue{extended union}\index{extended union}  
