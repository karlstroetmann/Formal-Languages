...
Next, we discuss the generation of code.\index{code generation}  We structure our representation by discussing the code generation for
...
\item \mytt{st} is the \blue{symbol table}: \index{symbol table} Concretely, this is a dictionary that maps
      ...
      positions of the variables in the local variable frame are assigned.
...
\item The second component of the return value of \mytt{compile\_expr} is a natural number.
      ...
      \\[0.2cm]
      \hspace*{1.3cm}
      \mytt{.limit stack} \mytt{\;\textsl{sz}}
      \\[0.2cm]
      where \mytt{\textsl{sz}} is the maximum size of the stack.  Controlling the 
      maximum height of the stack is a security feature of \textsl{Java} that prevents exploits that
      utilize stack overflows.
\end{enumerate}
All assembler commands that are generated are indented by eight space characters, while labels are indented by
four space characters.  This makes the generated assembler code easier to read.  The indentation of the commands
is done by the function \mytt{indent} that is shown in Figure \ref{fig:Compiler.ipynb:indent}.
...
\end{minted}
\vspace*{-0.3cm}
\caption{Indentation of assembler commands.}
\label{fig:Compiler.ipynb:indent}
\end{figure}

\noindent
The function \texttt{compile\_expr} is implemented via a case distinction.  Figure
\ref{fig:Compiler.ipynb:compile_expr} on page \pageref{fig:Compiler.ipynb:compile_expr} shows the skeleton of
this function.  In the following, we discuss the implementation of the different expressions one by one.
...
\end{minted}
\vspace*{-0.3cm}
\caption{Translation of an expression.}
\label{fig:Compiler.ipynb:compile_expr}
\end{figure}
...
\end{minted}
\vspace*{-0.3cm}
\caption{Translation of a constant.}
...
\textsl{lhs} and \textsl{rhs} are placed on the stack.  We add these values using the command \mytt{iadd}.
...
translation of arithmetic operators is implemented.  The translation of the subtraction, the multiplication, the
division, and the modulo operator is similar.  Note how we use the matching statement to extract the arithmetic
operator and the dictionary \texttt{OpToCmd} to pick the correct assembler command.
...
\item If $f$ is the function \mytt{println}, then \textsl{Jasmin} treats the function $f$ as a method of the
      ...
      \\[0.2cm]
      \hspace*{1.3cm}
      $s := \max(1 + s_1, 2 + s_2, \cdots, i + s_i, \cdots, n + s_n)$.
      \\[0.2cm]
      In this case we have to define the \blue{signature}\index{signature of a method} \mytt{fs} of $f$ as the string
      \\[0.2cm]
      \hspace*{1.3cm}
      ...
      \mytt{[} \; \mytt{invokevirtual}\; \mytt{fs} \;\mytt{]}, s\bigr\rangle$.

      Figure \ref{fig:Compiler.ipynb:compile:call_println} on page \pageref{fig:Compiler.ipynb:compile:call_println}
      shows how the translation of a call to \texttt{println} is implemented.
...
def compile_expr(expr, st, class_name):
    match expr:
        ...
        case 'call', 'println', *args:
            CmdLst    = [
                indent('getstatic java/lang/System/out Ljava/io/PrintStream;')
            ]
            stck_size = 0
            cnt       = 0
            for arg in args:
                ...
                cnt      += 1
            CmdLst += [
                indent(f'invokevirtual java/io/PrintStream/println({"I"*cnt})V')
            ]
            return CmdLst, stck_size
        ...
\end{minted}
\vspace*{-0.3cm}
\caption{Translation of a call to \texttt{println}.}
\label{fig:Compiler.ipynb:compile:call_println}
\end{figure}
\pagebreak
...
\item $f$ is a user defined function.  In this case, the byte code treats $f$ as a static function of the
      ...
      \hspace*{3.3cm}
      $\bigl\langle L_1 + \cdots + L_n + \mytt{[} \; \mytt{invokestatic}\; \mytt{fs} \;\mytt{]},
      \max(s,1)\bigr\rangle$.

      We have to take the maximum of $s$ and $1$ to cover the case that the function does not take any
      parameters.
      
      Figure \ref{fig:Compiler.ipynb:compile:call} on page \pageref{fig:Compiler.ipynb:compile:call} shows how the
...
Boolean expressions are built from equations and inequations using the logical operators
...
expect that the execution of these commands either puts the number $1$ (representing \mytt{True}) or the
number $0$ (representing \mytt{False}) onto the stack. 
The translation of Boolean expressions needs labels.  These labels are created by the function
\mytt{new\_label} that is shown in Figure \ref{fig:Compiler.ipynb:new_label}.  The global variable
\mytt{label\_counter} counts the labels that have already been created, so that every call of \mytt{new\_label}
returns a new label of the form \mytt{l1}, \mytt{l2}, \mytt{l3}, etc.
...
\end{minted}
\vspace*{-0.3cm}
\caption{Creating new labels.}
\label{fig:Compiler.ipynb:new_label}
\end{figure}
...
\\[0.2cm]
\hspace*{1.3cm}
$\mytt{compile\_expr}(\textsl{lhs}, \mytt{st}, \mytt{class\_name}) = \pair(L_1, s_1)$ \quad and \\
\hspace*{1.3cm}
...
\item Next, the control flow jumps to the label \textsl{next}, which is at the end of the generated
      list of assembler commands.  Hence, in this case the evaluation of the equation
...
into assembler code, we first have to recursively translate the expressions \textsl{lhs} and
\textsl{rhs} into assembler code.  Later, when this code is executed the values of the expressions
\textsl{lhs} and \textsl{rhs} are placed on the stack.  We combine these values using the command \mytt{iand}.
If the evaluation of \textsl{lhs} needs $s_1$ words on the stack and the evaluation of \textsl{rhs} needs $s_2$
words on the stack, then the evaluation of $\textsl{lhs} \;\mytt{\&\&}\; \textsl{rhs}$ needs 
\\[0.2cm]
\hspace*{1.3cm}
$\max(s_1, 1 + s_2)$
\\[0.2cm]
...
\rightarrow & \mytt{compile\_bool}(\textsl{lhs} \;\mytt{\&\&}\; \textsl{rhs}, \mytt{st}, \mytt{name}) = 
...
what happens in the language \mytt{C}.  In \mytt{C}, the evaluation of logical operators is \blue{lazy}\index{lazy evaluation}:
...
be the same as the size of the stack after the statement has been executed.
...
\\[0.2cm]
\hspace*{1.3cm}
$\mytt{compile\_bool}(\textsl{bool\_expr}, \mytt{st}, \mytt{class\_name}) = \pair(L_1, s_1)$
\\[0.2cm]
and, furthermore, 
\\[0.2cm]
\hspace*{1.3cm}
...
\begin{enumerate}
\item $\mytt{compile\_bool}(\textsl{bool\_expr}, \mytt{st}, \mytt{class\_name}) = \pair(L_1, s_1)$
...
\hspace*{0.0cm}
$ \textsl{compile\_stmnt}\bigl(\mytt{if}\;
     (\textsl{bool\_expr})\;\textsl{stmnt}_1\;\mytt{else}\;\textsl{stmnt}_2, \mytt{st},
     \mytt{class\_name}\bigr) \; = $
\\[0.2cm]
\hspace*{1.3cm}
$\bigl\langle  L_1 \;+\;  
    [\;\mytt{ifeq}\;\textsl{else}\;] \;+\;
    ...
    [\;\textsl{else}\mytt{:}\;] \;+\;
    L_3 \;+ 
    [\;\textsl{next}\mytt{:}\;],\; \max(s_1, s_2, s_3) \bigr\rangle        
$
\\[0.2cm]
...
def compile_stmnt(stmnt, st, class_name):
    match stmnt:
        ...
        case 'if-else', expr, then_stmnt, else_stmnt:
            ...
            CmdLst = L1 + [if_stmnt] + L2 + [goto_stmnt, else_stmnt] + \
                     L3 + [next_stmnt]
...
\\[0.2cm]
\hspace*{1.3cm}
$\mytt{compile\_bool}(\textsl{cond}, \mytt{st}, \mytt{class\_name}) = \pair(L_1, s_1)$
\\[0.2cm]
and
\\[0.2cm]
\hspace*{1.3cm}
...
def compile_stmnt(stmnt, st, class_name):
    match stmnt:
        ...
        case 'while', expr, body_stmnt:
            ...
            CmdLst = [loop_stmnt] + L1 + [if_stmnt] + \
                     L2 + [goto_stmnt, next_stmnt]
            return CmdLst, max(sz1, sz2)
        ...
\end{minted}
\vspace*{-0.3cm}
...
\[
  \begin{array}{l}
     ...
     \mytt{st}, \mytt{class\_name}\bigr) := \\[0.2cm]
     \hspace*{1.3cm} \bigl\langle L_1 + \cdots + L_n, \max(s_1, \cdots, s_n) \bigr\rangle.
  \end{array}
\]
...
\end{minted}
\vspace*{-0.3cm}
\caption{Translation of a block statement.}
...
In order to understand how this works we have to understand what type of code the function \mytt{compile\_fct} has
...
\item If the function that  is to be compiled has the name \squoted{main}, the code that has to be generated
      ...
      \begin{tcolorbox}[colback=sepia, boxrule=1pt, sharp corners, boxsep=0pt]
...
\item The \mytt{for} loop in line 12--14 initializes the symbol table \mytt{st}.  The parameters occupy the
      first positions of the local variable frame and the local variables follow the parameters.
...
\item The \mytt{for} loop in line 17--20 translates the statements in the body of the function one by one.
  Furthermore, it computes the size of the stack that is needed.
\item Line 21 declares the size of the local variable frame.  The pseudo-command \mytt{.limit locals}
  reserves memory for the parameters and the local variables.
\item Line 22 declares the size of the stack needed by the function.
\item Line 23 creates the command \mytt{return} that has to terminate the function \mytt{main}, since this
...
\item In line 1 we import the module \mytt{os}.  This module contains the submodule \mytt{path} which
      ...
      \item The function \mytt{dirname} takes a string specifying a \textsl{file path}, e.g.~something like
            \\[0.2cm]
            \hspace*{1.3cm}
            \mytt{'\symbol{126}/Formal-Languages/Python/Chapter-13/Examples/Test.c'} 
            \\[0.2cm]
            and returns the name of the directory.  In the example given above it would return the string
            \\[0.2cm]
            \hspace*{1.3cm}
            \mytt{'\symbol{126}/Formal-Languages/Python/Chapter-13/Examples'}. 
...
\item After computing the \mytt{directory} of the file and its \mytt{base} in line 4 and 5, we remove the 
      file extension \mytt{'.c'} in line 6.  Line 7 computes the name of the output file.  The function
      \mytt{os.path.join} combines the name of the directory and the name of the file using the separator of
      the operating system.
\item Line 8 and 9 read the file and store its content into the string \mytt{program}.
\item In line 10 the function \mytt{parse\_program} shown in Figure \ref{fig:Compiler.ipynb-6} converts the
      program into an abstract syntax tree that is represented as a nested tuple.  
...
\item The \mytt{for} loop in line 13--15 translates all function definitions into assembler code
      and concatenates the assembler statements into the list \mytt{CmdLst}.
\item In line 16 the output file is opened for writing.
\item Line 17--23 write the initialization code to the output file.
\item The \mytt{for} loop in line 24--25 writes the generated assembler commands to the output file.
\end{enumerate}
...
\exerciseEng
Extend the compiler that is presented in this chapter and that is available at
\\[0.2cm]
\hspace*{0.3cm}
\href{https://github.com/karlstroetmann/Formal-Languages/blob/master/Python/Chapter-13/Compiler.ipynb}{\scriptsize\texttt{github.com/karlstroetmann/Formal-Languages/blob/master/Python/Chapter-13/Compiler.ipynb}}
\\[0.2cm]
