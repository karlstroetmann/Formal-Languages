...
\exercise
Zeigen Sie, dass der String ``\texttt{1*2+3}'' in der Sprache der von der in
...
In this section we present a simple implementation of Earley's algorithm in \textsl{Python}.  
...
\item \texttt{mStateList} is the list $[Q_0, Q_1, \cdots, Q_n]$.  Since in \textsl{Python} lists are
...
name this method \texttt{scan} instead of \texttt{myScan} since \texttt{scan} is a keyword in \textsl{Python}.)
\begin{enumerate}
