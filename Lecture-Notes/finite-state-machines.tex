...
$Q_k$ \blue{inbetween}.  Here, ``inbetween'' specifies that the states $p$ and $q$ may well be elements of the set
...
\item[B.C.:] $|w| \leq 1$.  Then there are two cases:
  ...
  \item $\delta(p, c) = q \;\Rightarrow\; p \stackrel{c}{\mapsto}_k q$,
        ...
        to the state $q$, there are no states `ìnbetween''.
  \end{enumerate}
...
\item[B.C.:] $k = 0$.  
  ...
  states inbetween.  There are two cases.
  \begin{enumerate}
...
$Q_{n+1}$ inbetween.   Since we have
\\[0.2cm]
\hspace*{1.3cm}
...
$r$ specifies the language accepted by $F$.
...
Hence the notions of \blue{finite state machines} and \blue{regular expressions} are equivalent.

\section{Check your Understanding}
\begin{enumerate}
\item How are non-deterministic finite state machines defined?
