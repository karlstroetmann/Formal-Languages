...
$F = \langle Q, \Sigma, \delta, q_0, A \rangle$.
\\[0.2cm]
Without loss of the generality we want to assume that the \textsc{Fsm} $F$ is complete: Therefore, we assume that
...
string $s$ in the state $q$.  Since the function $\delta^*$ is a generalization of the function $\delta$, in
...
identified and consider all other states as equivalent.
...
\\[0.2cm]
\hspace*{1.3cm}
$\delta(p,c) = s$, $\delta(q,c) = t$ and $\langle s, t \rangle \in V$
\\[0.2cm]
