...
dass der Automat $F$ vollständig ist: Wir nehmen also an, dass der Ausdruck $\delta(q, c)$
...
$F^- = \langle Q^-, \Sigma, \delta^-, q_0, A^- \rangle$,
\\[0.2cm]
der dieselbe Sprache akzeptiert wie der Automat $F$, für den also
\\[0.2cm]
\hspace*{1.3cm}
$L(F^-) = L(F)$
\\[0.2cm]
...
Zunächst erinnern wir an die Funktion 
\\[0.2cm]
\hspace*{1.3cm}
...
$F = \langle Q, \Sigma, \delta, q_0, A \rangle$  alle die Zustände $p \in Q$ entfernen,
...
\begin{enumerate}
\item Zunächst initialisieren wir $V$ mit allen den Paaren $\pair(p,q)$, für die entweder
...
\item Solange wir ein neues Paar $\pair(p,q) \in Q \times Q$ finden, für dass es einen 
      Buchstaben $c$ gibt, so dass die Zustände $\delta(p,c)$ und $\delta(q,c)$ 
...
so können wir anschlie{\ss}end alle Zustände $p$ und $q$  identifizieren, die nicht
...
fügen wir au{\ss}erdem in der $j$-ten Zeile und der $i$-ten Spalte ebenfalls eine $1$ ein.
\begin{enumerate}
...
\item Als nächstes erkennen wir, dass die Zustände $q_0$ und $q_1$ unterscheidbar sind,
      ...
      \\[0.2cm]
      Genauso sehen wir, dass die Zustände $q_0$ und $q_2$ unterscheidbar sind, 
      denn es gilt 
      \\[0.2cm]
      \hspace*{1.3cm}
...
\item Nun finden wir keine weiteren Paare von unterscheidbaren Zuständen mehr,
      ...
      \\[0.2cm]
      aber da die Zustände $q_3$ und $q_4$ bisher nicht unterscheidbar sind,
