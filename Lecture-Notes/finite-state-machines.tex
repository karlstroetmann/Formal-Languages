...
A \blue{deterministic finite automaton} (abbreviated as \simtextsc{Dfa}\index{DFA@\simtextsc{Dfa}}) is a 5-tuple 
\\[0.2cm]
\hspace*{1.3cm}
...
\blue{reflexive-transitive closure}\index{reflexive-transitive closure} of the relation $\leadsto$ is written as $\leadsto^*$.
...
$\sim$ on the set of states $Q$.  Basically, $p_1, p_2\in Q$ are \blue{equivalent}\index{equivalent states} if they serve the same
purpose in the \simtextsc{Dfa} $F$, whatever that means.  If $p_1$ and $p_2$ are equivalent, this will be written as
\\[0.2cm]
\hspace*{1.3cm}
$p_1 \sim p_2$.
\\[0.2cm]
Given a state $p \in Q$, the \blue{equivalence class}\index{equivalence class} of $p$ w.r.t.~the relation $\sim$, written $[p]_\sim$ is the set of all states that
