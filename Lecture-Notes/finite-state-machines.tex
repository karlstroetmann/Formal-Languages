...
ist, dass der Automat intern eine fest vorgegeben Anzahl von Zuständen hat, in denen er sich
...
spezifizierte Sprache erkennt.
 ...
 Zustände, die wir mit den Zahlen $\{0,1,2, \cdots, 7\}$ durchnumerieren.  Im Folgenden
 ...
 erkläutert, das Ergebnis der Rechnung ist in Abbildung   \ref{fig:abstarbabab.dot} auf Seite
 ...
 \begin{figure}[!ht]
   \centering
    ...
    \caption{Ein endlicher Automat für die Sprache, die durch den regulären Ausdruck
...
geraten,  dass er in den Zustand $q_1$ wechseln muss.  Wäre der Automat hier im Zustand
$q_0$ verblieben, so könnte der akzeptierende Zustand $q_3$ nicht mehr erreicht werden:
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
Hier ist der Automat zu früh in den Zustand $q_1$ gewechselt, was bei der Eingabe des
letzten Zeichens zum Tode des Automaten führt.  Wäre der Automat beim Lesen des zweiten
Buchstabens \texttt{b} noch im Zustand $q_0$ geblieben, so hätte er das Wort
...
solchen spontanen Übergäng vom Zustand $q_1$ in den Zustand $q_2$ als
\\[0.2cm]
\hspace*{1.3cm}
...
\exercise
Geben Sie einen NEA $A$ an, so dass $L(F)$ aus genau den Wörtern der Sprache $\{a,b\}^*$ besteht, 
...
der Automat $A$ befinden könnte.  Die Zustände des deterministischen Automaten $\textsl{det}(A)$ sind also
...
\item An dem Eingabe-Alphabet ändert sich nichts, denn der neue Automat $\textsl{det}(A)$ soll ja die
...
\item Da $\textsl{ec}(q_0) = \{q_0\}$ gilt, besteht der Start-Zustand des
      deterministischen Automaten aus der Menge, die nur den Knoten $q_0$ enthält:
      \\[0.2cm]
      \hspace*{1.3cm}
      $S_0 := \textsl{ec}(q_0) = \{ q_0 \}$.
      \\[0.2cm]
      Wir bezeichnen den Start-Zustand mit $S_0$.
\item Von dem Zustand $q_0$ geht der nicht-deterministische Automat $A$ beim Lesen von \texttt{a} in den Zustand $q_0$ über.
      Also gilt
      \\[0.2cm]
      \hspace*{1.3cm}
...
Zustände?  Der Grund ist, dass von dem Start-Zustand $q_0$ nur solche Mengen von Zuständen
erreichbar sind, die den Zustand $q_0$ enthalten, denn egal ob \texttt{a} oder \texttt{b}
eingegeben wird, kann der Automat $A$ von $q_0$ immer wieder in den Zustand $q_0$ zurück
wechseln.  Daher muss jede Menge von Zuständen, die von $q_0$ erreichbar ist, selbst
wieder $q_0$ enthalten.  Damit entfallen als Zustände von $\textsl{det}(A)$ alle Mengen von $2^Q$, die
$q_0$ nicht enthalten, wodurch die Zahl der Zustände gegenüber der maximal möglichen
...
\item Den Automaten $A(\emptyset)$ definieren wir als
      ...
      \\[0.2cm]
      Beachten Sie, dass dieser Automat keinerlei Transitionen hat, für die
      Zustands-Übergangs-Funktion $\delta$ gilt also sowohl
      \\[0.2cm]
      \hspace*{1.3cm}
...
\item Um den Automaten $A(r_1 \cdot r_2)$ für die Konkatenation $r_1 \cdot r_2$ definieren
      ...
      wir immer erreichen, indem wir die Zustände des Automaten $A(r_2)$ umbennen.
      ...
      resultierende Autmat dann etwas kleiner wird.  Bei den praktischen Übungen werden
      wir diese Zustände daher identifizieren.
\item Um den Automaten $A(r_1 + r_2)$ definieren zu können, nehmen wir wieder an, dass die
      ...
      \end{enumerate}
      Gegenüber den Zustands-Übergänge der Automaten $A(r_1)$ und $A(r_2)$ kommen noch
      ...
      \end{enumerate}
      Um den so definierten Automaten zu vereinfachen, können wir einerseits die drei
...
\item Um den Automaten $A(r^*)$ für den Kleene-Abschluss $r^*$ 
      ...
      \end{enumerate}
      Zu den Zustands-Übergänge des Automaten $A(r)$  kommen 
      vier $\varepsilon$-Transitionen hinzu: 
      \begin{enumerate}
      \item Von dem neuen Start-Zustand $q_0$ gibt es jeweils eine
      ...
      \item Von $q_2$ gibt es eine  $\varepsilon$-Transition zu dem
            Zustand $q_3$.
      \end{enumerate}
      \textbf{Achtung}:  Wenn wir bei diesem Automaten versuchen würden, die Zustände
      ...
      Zustände dürfen also \textbf{nicht} identifiziert werden!
\end{enumerate}
...
$k=0,\cdots,n+1$ reguläre Ausdrücke $r^{(k)}(p_1, p_2)$.   Der reguläre Ausdruck
...
\item Für $k=1$ haben wir:
      ...
      \item Für $r^{(1)}(1, 1)$ finden wir
            ...
            \end{eqnarray*}
      \end{enumerate}
\item Für $k=2$ erhalten wir:
      \begin{enumerate}
      \item Für $r^{(2)}(0, 0)$ finden wir
        \begin{eqnarray*}
                  r^{(2)}(0, 0)
            & = & r^{(1)}(0, 0) + 
