...
\\[0.2cm]
\hspace*{1.3cm}
$\mathtt{Min}(F) := \bigl\langle \mathcal{Q}_\approx, \Sigma, \Delta, [q_0]_\approx, \widehat{A} \bigr\rangle$
\\[0.2cm]
...
\\[0.2cm]
and for which the number of states of the set $\mathcal{q}_\approx$ is minimal.  
...
\item[I.S.:] $k \mapsto k + 1$
     ...
     the pair  $\langle \delta(p,c), \delta(q,c) \rangle$ is already separable, then $\langle p, q \rangle$ is
     separable, too.  THerefore we define:  
     \\[0.2cm]
     \hspace*{1.3cm}
     $V_{k+1} := V_k \cup \bigl\{ \pair(p,q) \bigm| \exists c \in \Sigma: \langle \delta(p,c), \delta(q,c) \rangle \in V_k \rangle\bigr\}$
\end{enumerate}
...
\\[0.2cm]
\hspace*{1.3cm}
$p \sim q \; \stackrel{def}{\Leftrightarrow} \; \pair(p,q) \not\in V_K$.
\\[0.2cm]
