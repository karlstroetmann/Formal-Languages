...
\item The \textsc{Fsm} $A(\varepsilon)$ is defined as
      ...
      We have that $L\bigl(A(\varepsilon)\bigr) = \{\texttt{''}\}$, i.e.~the automaton only accepts the empty string. 
...
$r$ specifies the language accepted by $F$, i.e.~we have $L(r) = L(F)$.
 ...
 \section{Minimize Finate State Machines}
In this section we show a method to minimize the number of states of a deterministic finite state machine
\\[0.2cm]
\hspace*{1.3cm}
$F = \langle Q, \Sigma, \delta, q_0, A \rangle$.
\\[0.2cm]
Without limitation of the generality we want to assume that the automaton $F$ is complete: So we assume that the expression $\delta(q, c)$ 
returns for each state $q \in Q$ and each letter $c \in \Sigma$ as result a state of $Q$. Then we look for a deterministic finite automaton 
\\[0.2cm]
\hspace*{1.3cm}
$F^- = \langle Q^-, \Sigma, \delta^-, q_0, A^- \rangle$,
\\[0.2cm]
which accepts the same language as the machine $F$, so that therefore
\\[0.2cm]
\hspace*{1.3cm}
$L(F^-) = L(F)$
\\[0.2cm]
and for which the number of states of the set $Q^-$ is minimal.  
In order to be able to do this construction, we have to draw a bit out. First of all we remind of the function 
\\[0.2cm]
\hspace*{1.3cm}
$\delta^*: Q \times \Sigma^* \rightarrow Q$
\\[0.2cm]
which has as second argument not only a single letter it also accepts a string.
The function call $\delta^*(q,s)$ calculates the state $p$ which the automaton $F$ enters, if the machine reads the string $s$ in the state $q$.
Since the function $\delta^*$ is a generalization of the function $\delta$, we will not differentiate the notation between $\delta$ and $\delta^*$ and simply write $\delta$.

Obviously, in a finite state machine 
$F = \langle Q, \Sigma, \delta, q_0, A \rangle$ we can remove all the states $p \in Q$ which are not \blue{reachable} from the start state. That means a state $p$ is \blue{reachable}\index{reachable}, iff
a string $w \in \Sigma^*$ is given, so that
\\[0.2cm]
\hspace*{1.3cm}
$\delta(q_0, w) = p$
\\[0.2cm]
applies.  In the following we want to assume, that all states of the discussed finite automaton can be reached from the start state.
...
In general, we can minimize an automaton by identifying certain states.   
If we look for example at the automaton shown in figure \ref{fig:nicht-gleichwertig.dot}, we can identify the states $q_1$ and $q_2$ as well as $q_3$ and $q_4$ without changing the language of the automaton.
The central idea in minimizing an automaton is that we consider which states we are not allowed to identify at all and simply consider all other states as equivalent.
...
One possibility is to create a set $V$ with pairs of states.  We add the pair $\langle p, q \rangle$ to the set $V$ if we recognize that $p$ and $q$ are differentiable.  We recognize $p$ and $q$ as distinguishable if there is a letter $c\in\Sigma$ and two states $s$ and $t$, so that
\\[0.2cm]
\hspace*{1.3cm}
$\delta(p,c) = s$, $\delta(q,c) = t$ und $\langle s, t \rangle \in V$
\\[0.2cm]
applies. This idea provides an algorithm that uses two steps:
\begin{enumerate}
\item First we initialize $V$ with all the pairs $\pair(p,q)$, for which either $p$ is an accepting state and $q$ is not an accepting state, or reversed $q$ is an accepting state and $p$ is not an accepting state, because an accepting state can be distinguishabled from a non-accepting state by the empty string $\varepsilon$:
      \\[0.2cm]
      \hspace*{1.3cm}
      $V := \bigl\{\pair(p,q) \in Q \times Q \mid (p \in A \wedge q \notin A) \vee 
                                               (p \notin A \wedge q \in A) \bigr\}$
\item As long as we find a new pair $\pair(p,q) \in Q \times Q$ for which there is a letter $c$, so that the states $\delta(p,c)$ and $\delta(q,c)$ are already distinguishable, we add this pair to the set $V$: 
      \\[0.2cm]
      \hspace*{1.3cm} 
...
\\
      ...
      \texttt{\}}
\end{enumerate}
If we have found all pairs $\pair(p,q)$ of distinguishable states, then we can identify all states $p$ and $q$, which are not distinguishable and therefore $\langle p, q \rangle \not\in V$. It can be proved, that the automaton constructed in this way is in fact minimal.
\pagebreak

\vspace*{-0.5cm}
\exerciseEng
We consider the finite automaton shown in figure \ref{fig:nicht-gleichwertig.dot} and apply the algorithm described above to this automaton.
We use a table, whose columns and rows are numbered with the different states.  If we have recognized in the first step that the states $i$ and $j$ are distinguishable, we insert a $1$ in this table in the $i$-th row and the $j$-th column.
Furthermore, since the states $i$ and $j$ can also be distinguished from the states $j$ and $i$, we also insert a $1$ in the $j$-th row and the $i$-th column.
\begin{enumerate}
...
\item Now we do not find any more pairs of distinguishable states, because if we take a look at the pair $\pair(q_1,q_2)$ we can see that
      ...
      \\[0.2cm]
      a new distinguishable pair.  Now the two states $q_3$ and $q_4$ remain.  Here we find
      \\[0.2cm]
      \hspace*{1.3cm}
      ...
      \item $q_3 \sim q_4$
      \end{enumerate}
      Figure \ref{fig:gleichwertig.dot} shows the corresponding reduced finite automaton.
\end{enumerate}
...
\exerciseEng
Construct the minimal deterministic finite automaton that recognizes the language \linebreak
$L\bigl(a \cdot (b \cdot a)^*\bigr)$.  Take the following steps to do this:
\begin{enumerate}[(a)]
\item Calculate a non-deterministic finite state machine which recognizes this language.
\item Transform this automaton into a deterministic automaton.
\item Minimize the number of states of this automaton with the algorithm given above.
\end{enumerate}
