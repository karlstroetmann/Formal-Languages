...
A conflict is called a  \blue{mysterious reduce/reduce conflikt} if the conflict results from the merger of
...
chapter.  Then we had seen that this grammar is an LR grammar, but not LALR grammar.  Let us see what happens
...
Interestingly, \textsc{Ply} does not merge these states and can therefore produce a parse table without
conflicts.  On the other hand, \textsc{Ply} claims to generate LALR tables.  Therefore, I have have written an
email to \href{https://en.wikipedia.org/wiki/David_M._Beazley}{David Beaszley} asking whether this behaviour is
a feature or a bug.  So far I haven't received an answer.
