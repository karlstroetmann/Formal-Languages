...
As we have discussed similar programs at length in Chapter \ref{chapter:ply-lex} there is no need for further
discussions here.
...
grammar is implemented in \textsc{Ply}.  We discuss is line by line.
\begin{enumerate}
...
\item Line 5 -- 7 implement the grammar rule
      \\[0.2cm]
      \hspace*{1.3cm}
      ...
      a grammar rule (the \texttt{p} is short for \blue{parser}), $v$ should be the name of the variable
      ...
      \texttt{p\_$v$\_$s$} is a legal \textsl{Python} function name.

      The function always takes one argument \texttt{p}.  This argument is a sequence of objects that can be
      index with array notation. If the grammar rule defining $v$ has the form
      \\[0.2cm]
      \hspace*{1.3cm}
      ...
      \texttt{p[0]}.  In the grammar rule shown in line 5--7 there is no value associated with the grammar rule.
      ...
      \\[0.2cm]
      It is very \red{important} to note that the character ``\texttt{:}'' has to be surrounded by blank
      characters.  Otherwise, the parser generator will not work but rather generates error messages that are
      difficult to understand. 

      The function \texttt{p\_stmnt\_assign} has the task of evaluating the expression that is on the right
      hand side of the assignment operator ``\texttt{:=}''.  The result of this evaluation is the stored in the
      dictionary \texttt{Names2Values}.  The key that is used is the name of the identifier to the left of the
      assignment operator.  As \texttt{p[0]} is not set in this function, there is no value associated with a
      statement. 
...
\item The function \texttt{p\_expr\_plus} implements the grammar rule
      ...
      \\[0.2cm]
      It is implemented by evaluation the expression to the left of the operator ``\texttt{+}'' 
      and the product to the right of this operator and then adding the corresponding values.
      Finally, the sum is stored in \texttt{p[0]} so that it is available later.
...
is discussed next.
\begin{enumerate}
\item Line 1 -- 7 shows the function \texttt{p\_error} which is used to print error messages in case of a parse
      error.  The argument \texttt{p} is the token $t$ that caused $\texttt{action}(s, t)$ to be undefined.
