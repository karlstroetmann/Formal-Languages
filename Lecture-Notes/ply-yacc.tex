...
Of course, in this example shifting is wrong because then the string
\\[0.2cm]
\hspace*{1.3cm}
\texttt{1 - 2 + 3}
\\[0.2cm]
...
For the grammar shown previously we could add the following \blue{operator precedence declarations}:
\begin{verbatim}
    precedence = (
        ('left', '+', '-'),
        ('left', '*', '/'),
...
\item First, \textsc{Ply} assigns a precedence level to every grammar rule.  This precedence level is the
      precedence level of the last operator symbol occurring in the grammar rule.  Most of the times, there is
      operator would determine the precedence of the grammar rule. 
      In the grammar at hand the precedences of the rules would as shown in the table below. 
      \begin{center}
        \begin{tabular}[t]{|l|c|}
          \hline
          Regel                          & Präzedenz  \\
...
\item If $s$ is a state that contains two e.m.R.s  $r_1$ and  $r_2$ such that
      ...
      and the precedence of the rule $r_2$ is  $p(r_2)$.  Then there are five cases that depend
      ...
      \item $p(o) > p(c \rightarrow \gamma)$
            ...
            the rule, because it has to multiply the numbers $2$ and $3$.  Therefore, the token
            ``\texttt{*}'' has to be shifted.
      \item $p(o) < p(c \rightarrow \gamma)$
            ...
            \\[0.2cm]
            To see that this makes sense we discuss parsing of the expression
            \\[0.2cm]
            \hspace*{1.3cm}
            $\texttt{1*2+3}$
            \\[0.2cm]
            ...
            ``\texttt{1*2}'' using the rule $\textsl{expr} \rightarrow \textsl{expr} \squoted{*}
            \textsl{expr}$, as it has to multiply the numbers $1$ and $2$.  
      \item $p(o) = p(c \rightarrow \gamma)$ and the operator $o$ is left associative
      ...
      \item $p(o) = p(c \rightarrow \gamma)$ and the operator $o$ associates to the right 
            $c \rightarrow \gamma$
      ...
      \item $p(o) = p(c \rightarrow \gamma)$ and the operator $o$ has been declared to be non-associative
            ...
            the parser recognizes that there is an error.

            \remarkEng
            It is important to note that \textsc{Ply} will resolve this shift/reduce conflict by putting an
            error entry in the action table.
      \item $p(o)$ is undefined or $p(c \rightarrow \gamma)$ is undefined

            In this case there is a shift/reduce conflict and \textsc{Ply} prints a warning message when
            generating the parser.  Nevertheless, the conflict is resolved in favour of shifting.
      \end{enumerate}
\end{enumerate}
...
\href{https://github.com/karlstroetmann/Formal-Languages/tree/master/Ply/Look-Ahead/Look-Ahead.ipynb}{\texttt{Look-Ahead.ipynb}}\footnote{ 
