...
\\[0.2cm]
Damit ist klar, dass $L_1 \cup L_2$ eine reguläre Sprache ist. \qed
...
$L^R := \{ s^R \mid s \in L \}$.
\\[0.2cm]
Assume now that the language $L \subseteq \Sigma^*$ is regular.  Prove that then $L^R$ is a regular
language, too. \eox
...
\begin{Satz}
  ...
  gilt, ob also die von den beiden   Ausdrücken spezifizierte Sprachen gleich sind und damit
  \\[0.2cm]
  \hspace*{1.3cm}
  $L(r_1) = L(r_2)$
  \\[0.2cm]
...
\begin{Verbatim}[ frame         = lines, 
    ...
    fsmComplement := procedure(f1, f2) {
        ...
        delta  := procedure(q, c) {
                      [q1, q2] := q;
                      return [delta1(q1, c), delta2(q2, c)];
                  };
...
\item The function \texttt{regexp2DFA} takes a regular expression $r$ together
      ...
      The function \texttt{regexp2NFA} has already been shown in
...
\\[0.2cm]
\hspace*{1.3cm}
$q_0 \stackrel{c_1}{\mapsto} q_1 \stackrel{c_2}{\mapsto} q_2 \stackrel{c_3}{\mapsto} \cdots \stackrel{c_m}{\mapsto} q_m$
\\[0.2cm]
...
\item Beim Lesen von $u$ geht der Automat vom Zustand $q_0$ in den Zustand $q_k$ über, es gilt
      \begin{equation}
        \label{eq:pumping1}
        q_0 \stackrel{u}{\mapsto} q_k.    
      \end{equation}
\item Beim Lesen von $v$ geht der Automat vom Zustand $q_k$ in den Zustand $q_l$ über.  Da aber 
      $q_l = q_k$ ist, gilt also
      \begin{equation}
        \label{eq:pumping2}
      q_k \stackrel{v}{\mapsto} q_k.        
      \end{equation}
\item Beim Lesen von $w$ geht der Automat vom Zustand $q_l = q_k$   in den akzeptierenden Zustand
      $q_m$ über:
      \begin{equation}
        \label{eq:pumping3}
        q_k \stackrel{w}{\mapsto} q_m.        
      \end{equation}
\end{enumerate}
Aus $q_k \stackrel{v}{\mapsto} q_k$ folgt
\\[0.2cm]
\hspace*{1.3cm}
$q_k \stackrel{v}{\mapsto} q_k \stackrel{v}{\mapsto} q_k$, \quad also \quad $q_k \stackrel{v^2}{\mapsto} q_k$
\\[0.2cm]
Da wir dieses Spiel beliebig of wiederholen können, haben wir für alle $h \in \mathbb{N}$
\begin{equation}
  \label{eq:pumping4}
  q_k \stackrel{v^h}{\mapsto} q_k  
\end{equation}
Aus den Gleichungen (\ref{eq:pumping1}), (\ref{eq:pumping3}) und \ref{eq:pumping4})  folgt nun
\\[0.2cm]
\hspace*{1.3cm}
$q_0 \stackrel{uv^hw}{\mapsto} q_m$
\\[0.2cm]
...
\quad und \quad $\forall h \in \mathbb{N}: uv^h w \in L$
\\[0.2cm]
...
\\[0.2cm]
Setzen wir in der Formel $\forall h \in \mathbb{N}: uv^h w \in L$ für $h$ den Wert $0$ ein,
so sehen wir, dass 
\begin{equation}
  \label{eq:pumping5}
 uw \in L 
...
\\[0.2cm]
\hspace*{1.3cm}
$L_{\mathrm{square}} = \bigl\{ a^{m} \mid \exists k \in \mathbb{N}: m = k^2 \bigr\}$
\\[0.2cm]
Zeigen Sie mit Hilfe des Pumping-Lemmas, dass die Sprache $L_{\mathrm{square}}$ keine reguläre Sprache ist.
\vspace*{0.1cm}
...
\item Die Bedingung $v \not= \varepsilon$ liefert
      \begin{equation}
        ...
        y > 0.
      \end{equation}
\item Aus der Formel $\forall h \in \mathbb{N}: uv^hw \in L_{\mathrm{square}}$ folgt
      \begin{equation}
        \label{eq:e4}
...
\exercise
Es sei $\Sigma = \{\mathtt{a},\mathtt{b}\}$.  
Zeigen Sie, dass die Sprache 
\\[0.2cm]
\hspace*{1.3cm}
$L = \bigl\{ a^p \mid \mbox{$p$ is a prime number} \bigr\}$
\\[0.2cm]
 nicht regulär ist.
\vspace{0.3cm}
