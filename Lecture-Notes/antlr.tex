...
\\[0.2cm]
In dieser Vorlesung werden wir die Version 4.1 verwenden.
Zunächst führen wir \textsc{Antlr} an Hand verschiedener Beispiele ein, durch die wir die 
...
\item In Zeile 1 spezifizieren wir mit dem Schlüsselwort \texttt{grammar} den Namen der Grammatik.
      In unserem Fall hat die Grammatik also den Namen \texttt{Expr}.  Der Name  der Datei, in dem
...
\item Zeile 19 definiert schließlich das Token \texttt{WS}, wobei der Name als Abkürzung
      ...
      von \texttt{WS} folgt in geschweiften Klammern noch eine semantische Aktion.
      Die spezielle Aktion ``\texttt{skip}'', die hier ausgeführt wird, wirft das erkannte
      Token einfach weg, ohne es an den Parser weiterzureichen.
\end{enumerate}
...
\\[0.2cm]
\hspace*{1.3cm}
\texttt{java -jar /usr/local/lib/antlr-4.1-complete.jar Expr.g4}
\\[0.2cm]
Das setzt natürlich voraus, dass die angegebene Datei \texttt{antlr-4.1-complete.jar} auch
...
\item In den Zeilen 1 und 2 importieren wir die \textsc{Antlr}-Runtime-Pakete.
      Dort sind u.a.~die Klassen  \texttt{ANTLRInputStream} und \texttt{CommonTokenStream}, 
...
\\[0.2cm]
\hspace*{1.3cm}
\texttt{echo  \symbol{34}1 + 2 * 3 - 4\symbol{34} | java -cp /usr/local/lib/antlr-4.1-complete.jar ParseExpr}
\\[0.2cm]
...
\\[0.2cm]
anzeigen lassen.  Zum Aufruf von \textsc{Antlr} bietet such der Alias
\\[0.2cm]
\hspace*{1.3cm}
\texttt{alias antlr4='java -jar /usr/local/lib/antlr-4.1-complete.jar'}
\\[0.2cm]
...
\item Wir haben die Regeln für die syntaktischen Variablen \textsl{expr} und \textsl{product}
      vereinfacht.  Ein arithmetischer Ausdruck ist nichts anderes als eine Folge von Produkten,
      wobei die einzelnen Produkte durch die Operatoren ``\texttt{+}'' oder ``\texttt{-}'' getrennt
      werden.  Da \textsc{Antlr} \textsc{Ebnf}-Grammatiken unterstützt, können wir für \textsl{expr}
      daher die Grammatik-Regel
      \\[0.2cm]
      \hspace*{1.3cm}
      \texttt{expr : product (('+'|'-') product)* ;}
      \\[0.2cm]
      verwenden.  Die Grammatik-Regel für \textsl{product} folgt in ähnlicher Weise aus der
      Feststellung,  dass ein Produkt eine Folge von Faktoren ist, wobei die einzelnen Faktoren
      durch die Operatoren ``\texttt{*}'' und ``\texttt{/}'' getrennt werden.
...
\item In der Grammatik-Regel
      ...
      \texttt{expr : product (('+'|'-') product)* ;}
      \\[0.2cm]
      taucht die syntaktische Variable \textsl{product} zweimal auf. Deswegen wäre ein
      Notation der Form 
      \\[0.2cm]
      \hspace*{1.3cm}
      \texttt{\symbol{36}product.result} 
      \\[0.2cm]
      nicht eindeutig. Daher haben wir in Zeile 18 -- 20 den verschiedenen Auftreten dieser syntaktische
      Variablen die Namen $p$ und $q$ zugeordnet, auf die wir dann in den Aktionen
      zugreifen können.  In den Aktionen muss diesen Variablen allerdings ein Dollar-Zeichen
      vorgestellt werden.
...
\item In den Zeilen 34 und 35 greifen wir auf die Strings, die den Token
      \texttt{ID} und \texttt{INT} entsprechen, mit Hilfe der für Token vordefinierten
      Variable \texttt{text} zurück.
\end{enumerate}
...
\begin{figure}[htbp]
  ...
  \begin{eqnarray*}
  \textsl{Expr}   & \rightarrow & \;\textsl{Product}\;\;((\quoted{+}|\quoted{-})  \;\; \textsl{Product})* \\[0.2cm]
  \textsl{Product} & \rightarrow & \;\textsl{Factor}\;\;\;\;((\quoted{*}|\quoted{/}) \;\; \textsl{Factor})* \\[0.2cm]
...
\exercise
Auf meiner Webseite finden Sie unter
\\[0.2cm]
\hspace*{-0.3cm}
\href{http://wwwlehre.dhbw-stuttgart.de/~stroetma/Formal-Languages/Exercises/Grammar2HTML/c-grammar.g}{
\texttt{http://wwwlehre.dhbw-stuttgart.de/\symbol{126}stroetma/Formal-Languages/Exercises/Grammar2HTML/c-grammar.g}}
\\[0.2cm]
eine Grammatik für die Sprache \texttt{C}.
\begin{enumerate}
\item[(a)] Entwickeln Sie eine Grammatik, mit der Sie die Syntax der dort angegebenen
           Regeln beschreiben können.
\item[(b)] Entwickeln Sie mit Hilfe von \textsc{Antlr} ein Programm, dass die angegebene
           Grammatik liest und als \textsc{Html}-Datei ausgibt.  

           Zur Darstellung der Grammatik-Regeln finden Sie in dem Verzeichnis
           \\[0.2cm]
           \hspace*{-0.3cm}
           \href{http://wwwlehre.dhbw-stuttgart.de/~stroetma/Formal-Languages/Exercises/Grammar2HTML/}{
           \texttt{http://wwwlehre.dhbw-stuttgart.de/\symbol{126}stroetma/Formal-Languages/Exercises/Grammar2HTML/}}
           \\[0.2cm]
           verschiedene Klassen, deren \texttt{toString}-Methode \texttt{Html}-Ausgabe erzeugt.
\end{enumerate}
