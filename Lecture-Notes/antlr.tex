...
\textsc{Antlr} \index{\textsc{Antlr}} \cite{parr:2013,parr:2014}.  The name is short for 
...
\begin{enumerate}
\item In line 1 the keyword \green{\texttt{grammar}} specifies the name of the grammar.
...
\begin{figure}[!ht]
  \centering
      \epsfig{file=Abbildungen/parse-tree.png, scale=0.7}
...
\begin{enumerate}
\item Firstly, we present a grammar for a small \blue{symbolic calculator}.  Here, the attribute
      \emph{symbolic} means that the calculator supports the use of variables that can be bound to values.
\item Secondly, we show how this grammar can be extended with \blue{semantic actions} so that the expressions
      can be evaluated. 
\end{enumerate}
...
\begin{enumerate}
\item The start-symbol \texttt{program} now recognizes a list of \blue{statements}.
...
\end{verbatim}
Our next task is to develop an \blue{interpreter} for the language specified by the grammar shown 
...
\item Line 4 recognizes a sum of the form
      ...
      \\[0.2cm]
      and assign this triple to \texttt{\symbol{36}result}.  Note that we have to prefix
