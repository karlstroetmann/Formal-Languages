...
bei den  Parser-Generatoren \blue{\textsc{Antlr}} und \blue{\textsc{Ply}}, die wir später vorstellen werden. 
...
Bei den Terminalen gibt es zwei Arten:
\begin{enumerate}
\item Operator-Symbole und Trennzeichen wie beispielsweise ``\texttt{/}'' und
      ``\texttt{(}''.  

      Solche Terminale stehen für sich selbst.
...
\\[0.2cm]
\hspace*{1.3cm}
$w \in L \;\rightarrow\; \textsl{s} \Rightarrow^* w$.
\\[0.2cm]
...
\textsl{expr} & \Rightarrow & \textsl{expr} \quoted{*} \textsl{expr}                           \\
              ...
              & \Rightarrow & \texttt{2} \quoted{*} \texttt{3} \quoted{+} \texttt{4}           
\end{eqnarray*}
Diese Ableitung entspricht dem abstrakte Syntax-Baum, der in Abbildung
...
\blue{links-rekursiv}\index{links-rekursiv} ist, das heißt, dass eine Regel die Form
\\[0.2cm]
\hspace*{1.3cm}
$\textsl{a} \rightarrow \textsl{a}\, \beta$
\\[0.2cm]
hat, denn dann würden wir um ein $\textsl{a}$ zu parsen sofort wieder rekursiv versuchen, 
ein $a$ zu parsen und wären damit in einer Endlos-Schleife.  Es gibt zwei Möglichkeiten, um mit
diesem Problem umzugehen:
\begin{enumerate}
\item Wir können die Grammatik so umschreiben, dass sie danach nicht mehr links-rekursiv
      ist.
\item Eine einfachere Methode besteht darin, den Begriff der kontextfreien Grammatik
...
$
\\[0.2cm]
beschrieben wird.  Wir können diesen Fall durch die Einführung zweier Hilfs-Variablen $b$ und $c$ auf
