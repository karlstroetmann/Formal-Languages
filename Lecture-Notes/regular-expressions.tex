...
\centering
  \includegraphics[width=10.5cm]{Abbildungen/Stephen_Cole_Kleene.jpg}
\caption{Stephen Cole Kleene, the father of regular languages.}
\label{fig:Stephen_Cole_Kleene.jpg}
\end{figure}
...
programming languages (e.g.~\href{https://www.python.org/}{\textsl{Python}},
\href{https://isocpp.org/}{\texttt{C++}},
\href{https://dev.java/}{\textsl{Java}},
\href{https://learn.microsoft.com/en-us/dotnet/csharp/}{\texttt{C\#}},
\href{https://developer.mozilla.org/en-US/docs/Web/JavaScript}{\texttt{JavaScript}},
\href{https://www.typescriptlang.org/}{\texttt{TypeScript}},
\href{https://www.php.net/}{\texttt{PHP}},
\href{https://www.ruby-lang.org/}{\texttt{Ruby}},
\href{https://www.tcl-lang.org/}{\texttt{Tcl}}) support regular expressions.  In the programming language \texttt{C} regular
...
\begin{Definition}[Product of Languages]
  ...
  \blue{product}\index{product of languages}\glsadd{product} of $ L_1 $ and $ L_2 $ is denoted as $ L_1 \cdot L_2 $ and
...
Let $ \Sigma $ be an alphabet, $ L \subseteq \Sigma^* $ a formal language, and $ n \in \mathbb{N} $. The $ n $-th \blue{power} of $ L $, denoted as $ L^n $\glsadd{langpower}, is defined inductively as follows:
\begin{enumerate}
\item[B.C.:] $ n = 0 $:
...
\begin{Definition}[Kleene Closure]
  Given an alphabet $ \Sigma $ and a formal language $ L \subseteq \Sigma^* $, the \blue{Kleene
    closure}\index{Kleene closure}\glsadd{kleene} of $ L $, denoted as $ L^* $, is the union of all $ L^n $ for $ n \in \mathbb{N} $:
  \[
...
\begin{Definition}[Power of a string, $s^n$]\index{power of a string}\glsadd{strpower}
...
$L^*$ will be infinite whenever $L$ contains a string different from the empty string $\lambda$.
...
\\[0.2cm]
which interprets each regular expression $ r $ as a formal language $ L(r) \in 2^{\Sigma^*} $\glsadd{langofre}.\footnote{
  Given a set $ M $, the \blue{power set}\index{power set}\glsadd{powerset} of $ M $, i.e., the set of all subsets of $ M $, is denoted as $ 2^M $.
}

\begin{Definition}[Regular Expressions, Stephen Cole Kleene] \index{regular expression}
  The set $ \texttt{RegExp}_\Sigma $\glsadd{regexp} of \blue{regular expressions} on the alphabet $ \Sigma $ is defined inductively as follows:
  \begin{enumerate}
  \item $\emptyset \in \texttt{RegExp}_\Sigma$ \index{$\emptyset$}\glsadd{emptyre}
  ...
  \item $\varepsilon \in \texttt{RegExp}_\Sigma$ \index{$\varepsilon$}\glsadd{epsilon}
  ...
  \item $r_1 \in \texttt{RegExp}_\Sigma \wedge r_2 \in \texttt{RegExp}_\Sigma
         \rightarrow r_1 + r_2 \in \texttt{RegExp}_\Sigma$\glsadd{choice}
  ...
  \item $r_1 \in \texttt{RegExp}_\Sigma \wedge r_2 \in \texttt{RegExp}_\Sigma 
         \rightarrow r_1 \cdot r_2 \in \texttt{RegExp}_\Sigma$\glsadd{concat}

        Starting from the regular expressions $r_1$ and $r_2$ we can use the  infix operator
  ...
  \item $r \in \texttt{RegExp}_\Sigma \rightarrow r^* \in \texttt{RegExp}_\Sigma$\glsadd{star}
...
\\[0.2cm]
To remove this ambiguity, we assign \blue{operator precedences}\index{precedence!of regular expression operators} as follows:
\begin{enumerate}
...
\\[0.2cm]
Precedences do not tell us how to read an expression like $\mathtt{a} + \mathtt{b} + \mathtt{c}$, where the same
operator occurs twice.  We assume that the operators ``$+$'' and ``$\cdot$'' both associate to the left.
Therefore, 
\\[0.2cm]
\hspace*{1.3cm}
$a + b + c$
...
$(a \cdot b) \cdot c$
\\[0.2cm]
However, we will see in the next section that the operators ``$+$'' and ``$\cdot$'' are
associative, e.g.~the regular expression $(a+b)+c$ and $a + (b+c)$ are equivalent.
...
\exerciseEng
\begin{enumerate}[(a)]
\item Assume $ \Sigma = \{ \mathtt{a}, \mathtt{b}, \mathtt{c} \} $. Define a regular expression for the language $ L \subseteq \Sigma^* $ that consists of those strings that contain at least one occurrence of the letter ``\texttt{a}'' and at least one occurrence of the letter ``\texttt{b}''.
...
\\[0.2cm]
\hspace*{1.3cm}
$r_1 \doteq r_2 \quad \text{iff} \quad L(r_1) = L(r_2)$, \index{$\doteq$}\glsadd{doteq}
\\[0.2cm]
...
\item $(r^*)^* \doteq r^*$      
      ...
      such that for  $j=1,\cdots, m(i)$ there are strings $v_{j,i} \in L(r)$ satisfying
      \\[0.2cm]
      \hspace*{1.3cm}
...
\item $\varepsilon + r^* \cdot r  \doteq r^*$
\item $(\varepsilon + r)^* \doteq r^* $
\end{enumerate}
Used as rewrite rules, these laws can simplify regular expressions.  Most of them are read from left to right
for this purpose.  However, the laws of distributivity (g) and (h) have to be read from right to left: they are
used to factor out common parts, e.g.~$\mathtt{a} \cdot \mathtt{b} + \mathtt{a} \cdot \mathtt{c}$ is
replaced by $\mathtt{a} \cdot (\mathtt{b} + \mathtt{c})$.  The \textsl{Jupyter}
notebook
\\[0.2cm]
\hspace*{1.3cm}
\href{https://github.com/karlstroetmann/Formal-Languages/blob/master/Python/Chapter-02/Regular-Expressions.ipynb}{Python/Chapter-02/Regular-Expressions.ipynb}
\\[0.2cm]
contains a parser for the regular expressions defined in this chapter, a function \texttt{simplify\_regexp}
that simplifies regular expressions using the laws given above, and an interpreter for regular expressions:
the function \texttt{matches} checks whether a string is an element of the language $L(r)$ of a regular
expression $r$.
\pagebreak
