...
LALR-Parser erlauben es, auch links-rekursive Grammatiken in natürlicher Weise
zu parsen.  Da die meisten von Ihnen in der Praxis vermutlich mit \textsl{Java} arbeiten, 
möchte ich Ihnen in diesem Kapitel einen LALR-Parser-Generator vorstellen, den Sie
benutzen können, wenn Sie in \textsl{Java} programmieren.  Daher gebe ich Ihnen nun eine
kurze Einführung in die Verwendung des Parser-Generators \textsc{Cup} zusammen mit \textsl{JFlex}. 


Der Parser-Generator \textsc{Cup} \cite{hudson:1999}, der gelegentlich auch als \textsl{JavaCup}
bezeichnet wird, ist ein LALR-Parser-Generator für
...
\\[0.2cm]
Um einen mit \textsl{Cup} erzeugten Parser übersetzen zu können, benötigen Sie zusätzlich noch
die folgende Datei:
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
Eine \textsl{Cup}-Spezifikation besteht aus fünf Teilen.
\begin{enumerate}
\item Der erste Teil ist optional und enthält gegebenenfalls eine Paket-Deklaration.  
\item Der zweite Teil enthält die benötigten Import-Deklarationen.
...
\item Der vierte Teil ist wieder optional und spezifiziert  die Präzedenzen von Operator-Symbolen.
\item Der fünfte Teil enthält die Grammatik-Regeln.
\end{enumerate}
...
\item In den Zeilen 3 bis 5 werden die Terminale deklariert.  Es gibt zwei Arten von Terminalen:
      ...
      \item Terminale, die keinen zusätzlichen Wert haben.  Hierbei handelt es sich 
            ...
            deklariert.
      \item Terminale mit einem zusätzlichen Wert.  In diesem Fall muss zusätzlich der Typ
            dieses zusätzlichen Werts deklariert werden.  Die Syntax ist in diesem Fall
            \\[0.2cm]
            \hspace*{1.3cm}
...
\item Der letzte Teil einer \textsc{Cup}-Grammatik-Spezifikation enthält die Grammatik-Regeln. 
      ...
      \item Um auf die Werte, die einem Terminal oder einer syntaktischen Variablen zugeordnet sind,
            ...
            das Schlüsselwort ``\texttt{RESULT}''.  Betrachten wir als Beispiel die Regel
            \\[0.2cm]
            \hspace*{1.3cm}
...
\\[0.2cm]
übersetzen.  Damit dies funktioniert müssen Sie die Variable \texttt{CLASSPATH} so setzen, dass die
Klasse \texttt{java\_cup.Main} gefunden
werden kann.  Eine Möglichkeit, den Aufruf zu vereinfachen, besteht unter Unix darin, dass Sie sich
...
\end{verbatim}
In dieser Datei müssen Sie die Variable \texttt{CLASSPATH} natürlich so anpassen, dass die
Datei 
\\[0.2cm]
\hspace*{1.3cm}
\texttt{java-cup-11a.jar}
\\[0.2cm]
gefunden werden kann.  Danach können Sie \textsl{JavaCup} einfacher mit dem Befehl
\\[0.2cm]
\hspace*{1.3cm}
\textsl{cup calc.cup}
\\[0.2cm]
...
\begin{enumerate}
\item Die Datei \texttt{parser.java} enthält die Klasse \texttt{parser}, die den eigentlichen
...
\item Die Datei \texttt{sym.java} enthält die Klasse \texttt{sym}, welche die verschiedenen
...
Um mit dem Parser arbeiten zu können, brauchen wir eine Klasse, die eine Methode
$\texttt{main}()$ enthält.  Abbildung \ref{fig:Calculator.java} auf Seite
\pageref{fig:Calculator.java} zeigt eine solche Klasse.  Wir
erzeugen dort in Zeile 4 einen Parser, indem wir den Konstruktor der Klasse \texttt{parser} mit
...
$\textsl{parse}()$ gestartet, wobei eventuelle Ausnahmen noch abgefangen werden müssen.
Falls mit dem Start-Symbol der Grammatik ein Wert assoziert ist, so wird dieser Wert von
der Methode $\textsl{parse}()$ als Ergebnis zurück gegeben, andernfalls wird der Wert
\texttt{null} zurück gegeben.
...
\item Zeile 8 spezifiziert mit dem Schlüsselwort ``\texttt{\%cup}'', 
...
\item In den Zeilen 11 bis 17 definieren wir zwei Hilfs-Methoden, die Objekte vom Typ 
      ...
      \item \texttt{public Symbol(int \textsl{symbolID});}
            ...
            Jedes Terminal und jede syntaktische Variable entspricht genau Symbol-Nummer. 
      ...
      \item \texttt{public Symbol(int \textsl{symbolID}, int  \textsl{start}, int \textsl{end},
            ...
            den Wert, den dieses Token hat.
      \end{enumerate}
      Bei der Implementierung der Hilfs-Methoden $\texttt{symbol}()$ verwenden wir die Funktion
      $\texttt{yylength}()$.  Diese Funktion gibt die Länge des Strings zurück, der dem zuletzt
      erkannten Tokens entspricht.
\item In den Zeilen 22 bis 29 erkennen wir die arithmetischen Operatoren und die Klammer-Symbole.
      Wir verwenden dabei die in der Klasse \texttt{sym} definierten Konstanten.
\item In Zeile 31 erkennen wir mit dem regulären Ausdruck ``\texttt{0|[1-9][0-9]*}'' eine natürliche
      Zahl.  Diese verwandeln wir durch Aufruf des Konstruktors
      \\[0.2cm]
      \hspace*{1.3cm}
      \texttt{new Integer(yytext())}
      \\[0.2cm]
      in ein Objekt vom Typ \texttt{Integer}, wobei der Text, der der Zahl entspricht, von der
      Funktion $\texttt{yytext}()$ geliefert wird.  Anschließend geben wir ein Symbol zurück,
      in dem dieses Objekt als zugehöriger Wert abgespeichert wird.
\item In Zeile 33 überlesen wir Leerzeichen, Tabulatoren und Zeilen-Umbrüche.
...
\end{enumerate}
\textsl{JFlex} erzeugt den Scanner in der Klasse \texttt{Yylex}.  Wir hatten in Abbildung
...
Wir betrachten nun ein weiteres Beispiel.  Abbildung \ref{fig:calc-ambiguous.cup} zeigt eine Grammatik, die
offenbar mehrdeutig ist. 
...
\\[0.2cm]
gelesen werden soll.   Wir hatten im letzten Kapitel schon gesehen, dass es in einer mehrdeutigen Grammatik
immer Shift-Reduce- oder Reduce-Reduce-Konflikte geben muss.
Wenn wir versuchen, diese Grammatik mit \textsc{Cup} zu übersetzen und wenn wir dabei zusätzlich die Option 
``\texttt{-dump}'' angeben, erhalten wir eine große Zahl von Shift-Reduce-Konflikten angezeigt.
...
worden ist.  Die Menge aller Zustände kann über die Option ``\texttt{-dump}'' mit
ausgegeben werden.   Dazu wird \textsc{Cup} in der Form
\\[0.2cm]
\hspace*{1.3cm}
...
Regel $R_1$ liegen kann, ist an dieser Stelle unklar, ob das Token ``\texttt{+}'' auf den
...
\section{Operator-Präzendenzen}
Es ist mit   \textsc{Cup} möglich, Shift-Reduce-Konflikte durch die Angabe von \emph{Operator-Präzedenzen}
...
gelesen.  Demgegenüber ist der Operator ``\texttt{\^}'', der in der \textsc{Cup}-Grammatik
mit ``\texttt{POW}'' bezeichnet wir und die Potenzbildung bezeichnet,
\emph{rechts-assoziativ}, der Ausdruck 
\[ 4 \texttt{\symbol{94}} 3 \texttt{\symbol{94}} 2 \quad \mbox{wird also als} \quad 4^{\mbox{(}3^2\mbox{)}} \quad 
   \mbox{und nicht als} \quad \bigl(4^3\bigr)^2
\]
...
\item Ist nun $s$ ein Zustand, in dem zwei Regeln $r_1$ und $r_2$ der Form
      ...
      dieser beiden Zahlen aufgelöst.  Hier werden fünf Fälle unterschieden:
      \begin{enumerate}
      ...
      \item $p(o) = p(B \rightarrow \gamma)$ und der Operator $o$ hat keine Assoziativität:
      ...
      werden kann, wird eine Warnung ausgegeben.  In diesem Fall wird der Konflikt dann dadurch aufgelöst, 
...
%\item Die zur Darstellung des Syntax-Baums benötigten Klassen sollen mit Hilfe des
%      Klassen-Generators \textsc{Ep} erzeugt werden.
\item Die Grammatik soll in lesbarer Form ausgegeben werden.  Dabei sollen die
      semantischen Aktionen unterdrückt werden.
...
\item \emph{Mehrdeutigkeits-Konflikte} sind Reduce-Reduce-Konflikte, die ihre Ursache in einer Mehrdeutigkeit
      ...
      wird. 
\item \emph{Look-Ahead-Konflikte} sind Reduce-Reduce-Konflikte, bei denen die Grammatik zwar eindeutig ist, 
...
\subsection{Mehrdeutigkeits-Konflikte}
Abbildung \ref{fig:ambiguity-conflict.cup} zeigt die \textsl{Cup}-Spezifikation einer
Grammatik, die offensichtlich mehrdeutig ist, denn das Token \textsc{Number} kann sowohl
als ein \texttt{a} als auch als ein \texttt{b} geparst werden.
...
\end{Verbatim}
\vspace*{-0.3cm}
\caption{Eine mehrdeutige Grammatik.}
\label{fig:ambiguity-conflict.cup}
\end{figure}

\noindent
\textsl{Cup} liefert bei dieser Grammatik die folgende Fehlermeldung:
\begin{verbatim}
...
\end{verbatim}
Die einzige Möglichkeit diesen Konflikt aufzulösen besteht darin, die Grammatik so
umzuschreiben, dass die Mehrdeutigkeit verschwindet.
...
\\[0.1cm]
gefunden.},
die zwar eindeutig ist, die aber keine LR-Grammatik ist.  
...
\\[0.2cm]
\hspace*{1.3cm}
$\{ B \rightarrow \quoted{x} \bullet: \squoted{u},\; C \rightarrow \quoted{x} \bullet: \squoted{u} \}$
\\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm}
$A \rightarrow B \quoted{u} \quoted{v}$
\\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm}
$A \rightarrow C \quoted{u} \quoted{w}$
\\[0.2cm]
...
entnommen. (\textsl{Bison} ist ein LALR-Parser-Generator für die Sprachen \texttt{C} und \texttt{C++}.)
