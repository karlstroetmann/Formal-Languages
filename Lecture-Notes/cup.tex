...
``\texttt{-}'', ``\texttt{*}'', ``\texttt{/}'' und ``\texttt{\symbol{94}}'' aufgebaut sind.   Mit Hilfe der
...
\\[0.1cm]
gefunden.},
die zwar eindeutig ist, aber trotzdem keine LR-Grammatik ist.  
...
\item Außerdem sind auch Listen von Ausdrücken möglich.
\end{enumerate}
...
\end{Verbatim}
\vspace*{-0.3cm}
\caption{Eine \textsl{JavaCup}-Grammatik für $\lambda$-Ausdrücke.}
\label{fig:lambda.g}
\end{figure}

Versuchen wir mit \textsl{JavaCup} einen Parser für diese Grammatik zu erzeugen, so erhalten wir
