...
development of scanners,%
\footnote{A \emph{scanner} (also called a \emph{lexer} or \emph{tokenizer}) splits a text into \emph{tokens}
  such as numbers, names, keywords, and operators, e.g.~``\texttt{x1 + 23}'' is split into ``\texttt{x1}'', ``\texttt{+}'',
  and ``\texttt{23}''.}
parsers,%
\footnote{A \emph{parser} checks whether a sequence of tokens conforms to the \emph{grammar} of a language and
  builds a tree that shows the structure of the input.}
interpreters,%
\footnote{An \emph{interpreter} executes a program directly, instruction by instruction.  For example
  \textsl{Python} programs are executed via an interpreter.}
and compilers.%
\footnote{A \emph{compiler} translates a program into a lower-level language like machine code
  or \textsl{Java} byte code, which is executed later.}
We also introduce a tool designed to aid in the
