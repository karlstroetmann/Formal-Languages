...
\item We then demonstrate how to create \blue{scanners} using regular expressions directly in Python.
...
\item Next, we talk about using \blue{\simtextsc{Lark}} as a parser generator.
...
\item We also explore advanced features of \simtextsc{Lark}, such as resolving shift-reduce and reduce-reduce conflicts.
\end{enumerate}
\item The final part of this lecture covers interpreters and compilers.
\begin{enumerate}[(a)]
\item We implement an interpreter for a toy programming language using \simtextsc{Lark}.
\item Then we discuss how to implement a compiler that translates a subset of the \texttt{C} language (Integer-C) into \blue{Java byte code} via \blue{Jasmin} assembler.
\end{enumerate}
\end{enumerate}
\section{Literature}
