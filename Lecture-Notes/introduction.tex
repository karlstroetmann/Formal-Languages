...
\item \( \Sigma_{\simtextsc{\scriptsize Unicode}} = \{ 0, 1, \cdots, 172\,808 \} \) represents version 18.0 of the
...
emojis.  Table \ref{tab:chinese_table} shows the Unicode encoding of the 100 most common Chinese characters.
\eox
\end{enumerate}
\begin{table}[!th]
\centering
...
\( i < |w| \), \( w[i] \) specifies\glsadd{char} the \( i \)-th character in \( w \). Character counting starts at 0, adhering to
...
\item Next, we prove that regular languages are closed under union, intersection, and complement.  Using these
      \blue{closure properties}, we can decide whether a regular language is empty and whether two regular
      expressions are \blue{equivalent}.
