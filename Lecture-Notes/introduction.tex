...
For a given alphabet \( \Sigma \), a \blue{string} \index{string} is a sequence of characters selected from \( \Sigma \). In formal language theory, these sequences are written without brackets or separating commas. Thus, for \( c_1, \ldots, c_n \in \Sigma \), we write
\\[0.2cm]
\hspace*{1.3cm}
...
Given an alphabet \( \Sigma \), a subset \( L \subseteq \Sigma^* \) is termed a \blue{formal language} if it is \emph{precisely defined}. \index{formal language} \qed
\end{Definition}
