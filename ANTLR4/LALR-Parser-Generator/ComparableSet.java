               ...
               Iterable<T>
{
    ...
    private TreeSet<ComparableSet<T>> cloneSet(Set<ComparableSet<T>> set) {
    ...
    //   1. s <= M
    //   2. +/ { p(x) : x in M } <= M
    public static <S> TreeSet<S> fixpoint(Producer<S> p, TreeSet<S> s) {
        TreeSet<S> newElements = new TreeSet<S>();
        newElements.addAll(s);
        while (!newElements.isEmpty()) {
            TreeSet<S> moreElements = new TreeSet<S>();
            for (S x: newElements) {
                TreeSet<S> found = p.produce(x);
                for (S y: found) {
                    if (s.add(y)) {
                        moreElements.add(y);
                    }
