...
public class Grammar {
    ...
    public Grammar(List<Rule> rules) {
        ...
        follow.add(dollar);
        MarkedRule start = new MarkedRule(mStartRule, follow);
    ...
    */
    void augmentGrammar(List<Rule> rules) {
        ...
        MyToken dollar = new MyToken("$");
        fts.add(dollar);
        mAcceptRule = new MarkedRule(start, body2, body3, fts);
    ...
    // Argument uebergebenen Menge immer in solchen Bloecken auftreten, bei denen
    ...
    Set<MarkedRule> combine(Set<MarkedRule> rules) {
        if (rules.isEmpty()) {
            return rules;
        }
        Set<MarkedRule> result = new TreeSet<MarkedRule>();
        MarkedRule pr = null;   // previous rule
        ...
        for (MarkedRule r: rules) {
            ...
            if (pr.similar(r)) {
                ...
                MarkedRule nr = 
                    new MarkedRule(pr.getHead(), pr.getFirst(), pr.getSecond(), follow);
                result.add(nr);
                follow = r.getFollow();
                pr = r;
            }
        }
        result.add(new MarkedRule(pr.getHead(), pr.getFirst(), pr.getSecond(), follow));
        return result;
    }
    ...
    Set<MarkedRule> gotoFct(MarkedRule rule, Item t) {
        ...
        if (next.equals(t)) {
            ...
            MarkedRule newRule   = new MarkedRule(head, newFirst, newSecond, rule.getFollow());
            result.add(newRule);
        }
        return result;
    }
    ...
    Set<MyToken> result = computeFirst(li);
